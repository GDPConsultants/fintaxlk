...
// ── PWA: inject manifest + register service worker ───────────────────────────
  ...
  const props={data,update,profile,setProfile:saveProfile,tax,cashBalance,totalBankBal,bankBalance,trialDaysLeft,paidDaysLeft,onUpgrade:activatePaid,onActivateCode:activateWithCode,sub,incomeCats,handleLogoTap,lang,setLang,t,activeYear,switchYear};
...
function Paywall({onPay,onActivateCode,reason,profile,lang,setLang}){
  ...
  const waPaymentMsg=`Hello GDP Consultants 👋\n\nI have made payment for FinTax LK Annual Subscription.\n\nName: ${profile?.name||"[your name]"}\nMobile: ${profile?.mobile||"[mobile]"}\nAmount: Rs.${priceSetting.toLocaleString()}\nBank: ${BANK_DETAILS.bank}\nAcc: ${BANK_DETAILS.accountNo}\n\nPlease activate my subscription. Thank you!`;
  ...
  return(
    <div style={{...S.phone,overflowY:"auto",padding:20,justifyContent:"flex-start",alignItems:"center",background:"linear-gradient(160deg,#060d1a,#0a0a00)"}}>
      ...
      <div style={{width:"100%",boxSizing:"border-box",marginBottom:10}}>
        <button onClick={()=>{setShowCode(v=>!v);setCodeStatus(null);setCodeMsg("");setShowTransfer(false);setShowBank(false);}}
      ...
      <div style={{width:"100%",boxSizing:"border-box",marginBottom:10}}>
        <button onClick={()=>{setShowBank(v=>!v);setShowCode(false);setShowTransfer(false);}}
          style={{width:"100%",padding:"12px",background:showBank?"#1a1200":"linear-gradient(135deg,#1a2a0a,#14240a)",border:"2px solid "+(showBank?"#b8960c":"#4ade80"),borderRadius:12,color:showBank?"#b8960c":"#86efac",fontWeight:"800",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          🏦 Pay via Bank Transfer
        </button>
        {showBank&&(
          <div style={{background:"#0a1400",border:"2px solid #4ade80",borderTop:"none",borderRadius:"0 0 12px 12px",padding:"16px"}}>
            <div style={{fontSize:12,fontWeight:"800",color:"#86efac",marginBottom:12,textAlign:"center"}}>
              📋 Bank Transfer Details — Rs.{priceSetting.toLocaleString()}/year
            </div>
            {[
              ["Account Name", BANK_DETAILS.accountName],
              ["Account No.", BANK_DETAILS.accountNo],
              ["Bank", BANK_DETAILS.bank],
              ["Branch", BANK_DETAILS.address],
              ["SWIFT Code", BANK_DETAILS.swiftCode],
            ].map(([lbl,val])=>(
              <div key={lbl} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #14240a"}}>
                <span style={{fontSize:11,color:"#64748b"}}>{lbl}</span>
                <span style={{fontSize:11,fontWeight:"700",color:"#e2e8f0",textAlign:"right",maxWidth:"60%"}}>{val}</span>
              </div>
            ))}
            <div style={{background:"#0f2a18",border:"1px solid #166534",borderRadius:10,padding:"12px",marginTop:14}}>
              <div style={{fontSize:11,color:"#86efac",fontWeight:"700",marginBottom:8}}>📲 After Payment — WhatsApp Us:</div>
              <div style={{fontSize:10,color:"#4ade80",lineHeight:1.7,marginBottom:12}}>
                Send your payment slip/screenshot to our WhatsApp. Admin will generate an activation code and send it back within 2 hours (business hours).
              </div>
              <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${encodeURIComponent(waPaymentMsg)}`}
                target="_blank"
                style={{display:"block",background:"linear-gradient(135deg,#065f46,#047857)",color:"#fff",border:"none",borderRadius:10,padding:"12px",fontWeight:"800",fontSize:14,textDecoration:"none",textAlign:"center"}}>
                💬 Notify Us on WhatsApp
              </a>
              <div style={{fontSize:10,color:"#374151",marginTop:8,textAlign:"center"}}>
                Tap to open WhatsApp with payment details pre-filled
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{width:"100%",boxSizing:"border-box",marginBottom:14}}>
        <button onClick={()=>{setShowTransfer(v=>!v);setShowCode(false);setShowBank(false);}}
      ...
      {/* Tiered Plans - Individual & Company */}
      ...
      <div style={{...S.card,border:"1px solid #b8960c",width:"100%",boxSizing:"border-box",background:"linear-gradient(160deg,#0d0d00,#1a1500)",marginBottom:16}}>
        ...
        <div style={{textAlign:"center",fontSize:36,fontWeight:"900",color:"#f5d060"}}>Rs.{priceSetting.toLocaleString()}<span style={{fontSize:13,color:"#64748b",fontWeight:"400"}}>/year</span></div>
...
function AdminPanel({onExit,lang,setLang}){
  ...
  const [view,setView]=useState("codes"); // codes | all | reg | pay | pin | pricing | subs
  ...
  const refreshData=async()=>{
    const [l,c,p,s]=await Promise.all([
      load("admin_notif_log",true),load("fintax_codes",true),
  ...
  const handleDeactivateSub=async(subId)=>{
    ...
    await save(ADMIN_SUBSCRIBERS_KEY,subs,true);
    // Also store a deactivated flag in shared storage keyed by mobile
    ...
    <div style={S.phone}>
      ...
      <div style={{padding:"14px",overflowY:"auto",flex:1,paddingBottom:24}}>
        ...
        <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}}>
          {[["codes","🔑 Codes"],["subs","👥 Subs"],["pricing","💰 Pricing"],["all","📋 All"],["reg","👤 Users"],["pay","💳 Payments"],["pin","🔒 PIN"]].map(([v,l])=>(
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:4,flexShrink:0}}>
                      ...
                      {sub.active&&sub.mobile&&(
                        <a href={`https://wa.me/94${(sub.mobile||"").replace(/^0/,"")}?text=${encodeURIComponent(`Dear ${sub.name||"Subscriber"}, your FinTax LK subscription expires on ${sub.expiryDate}. Please renew to continue access. Pay Rs.${SUBSCRIPTION_PRICE.toLocaleString()} to: Sampath Bank, ${BANK_DETAILS.accountNo} (${BANK_DETAILS.accountName}). WhatsApp us with slip for activation. — GDP Consultants`)}`}
                          target="_blank"
  ...
  return(
    ...
    {renewalAlert&&(
      <div style={{background:"linear-gradient(135deg,#7f1d1d,#991b1b)",padding:"6px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexShrink:0}}>
        ...
        <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${encodeURIComponent(`Hello GDP Consultants, I need to renew my FinTax LK subscription. My name is ${profile?.name||""} and mobile is ${profile?.mobile||""}. Please provide renewal details.`)}`}
          target="_blank"
...
<div class="disclaimer">⚠️ <b>Disclaimer:</b> This report is generated from data entered in FinTax LK. Verify all figures with your GDP Consultants tax advisor before submission to IRD.</div>
      ...
      {section==="privacy"&&(
        <>
          ...
          <div style={{background:"linear-gradient(135deg,#0a0f1e,#0d1527)",border:"1px solid #1e3a5f",borderRadius:14,padding:14,marginBottom:12}}>
            ...
            <div style={{fontSize:11,color:"#64748b",lineHeight:1.7}}>
              FinTax LK is committed to protecting your financial privacy. This policy explains what data we collect, how it is used, and your rights.
            </div>
          </div>
          ...
          <div style={{...S.card,border:"1px solid #065f46",background:"#021a0e",marginBottom:10}}>
            ...
            <div style={{fontSize:11,color:"#94a3b8",lineHeight:1.8}}>
              <strong style={{color:"#e2e8f0"}}>FinTax LK does not maintain or store your transaction data on external servers.</strong><br/><br/>
          ...
          <div style={{...S.card,border:"1px solid #1e293b",background:"#080d18",marginBottom:10}}>
            ...
            {[
              ...
              {icon:"✅",title:"Google Drive Backup",desc:"If you use the backup feature, your data goes directly from your device to YOUR personal Google Drive. FinTax LK never sees or stores this data."},
