...
const APP_VERSION     = "14.0.0";  // bump this each release → triggers update banner
...
// ─── Multi-Year Support ────────────────────────────────────────────────────────
const ALL_TAX_YEARS = ["2022/2023","2023/2024","2024/2025","2025/2026"];
...
const ALL_STORAGE_KEYS = [
  ...
  ...["2022/2023","2023/2024","2024/2025","2025/2026"].map(y=>"fintax_v4_"+y.replace("/","_"))
];
...
const TRANSLATIONS = {
  en:{
    ...
    annual_plan:"SUBSCRIBE — BANK TRANSFER", pay_btn:"View Plans & Pay via Bank Transfer",
    ...
    tier_ind_basic:"Rs.4,800/yr",tier_ind_pro:"Rs.12,000/yr",tier_ind_prem:"Rs.24,000/yr",tier_co_start:"Rs.18,000/yr",tier_co_biz:"Rs.36,000/yr",tier_co_ent:"Rs.60,000/yr",
  ...
  si:{
    ...
    annual_plan:"රු. ගෙවා සක්‍රිය කරන්න", pay_btn:"සැලැස්ම තෝරා බැංකු හරහා ගෙවන්න",
    ...
    tier_ind_basic:"රු.4,800/වර්ෂ",tier_ind_pro:"රු.12,000/වර්ෂ",tier_ind_prem:"රු.24,000/වර්ෂ",
  ...
  ta:{
    ...
    annual_plan:"திட்டம் தேர்வு செய்யவும்", pay_btn:"திட்டம் தேர்ந்தெடுத்து வங்கி மூலம் செலுத்தவும்",
    ...
    tier_ind_basic:"ரூ.4,800/ஆண்டு",tier_ind_pro:"ரூ.12,000/ஆண்டு",tier_ind_prem:"ரூ.24,000/ஆண்டு",
...
const SUBSCRIPTION_PRICE = 8000; // Default: Individual Basic — admin selects plan when generating code
...
const BANK_ACCOUNTS = [
  {
    ...
    acName:"GDP CONSULTANTS",
    acNo:"101914037651",
    swift:"BSAMLKLXI",
  },
];

// ─── Pricing Plans — Individual vs Company ────────────────────────────────────
const PLANS = {
  individual: [
    { id:"ind_basic", name:"Basic",    badge:"",           color:"#64748b", border:"#334155",
      price:8000,  priceLabel:"Rs.\u200b8,000/yr",
    ...
    { id:"ind_pro",   name:"Pro",      badge:"POPULAR",    color:"#b8960c", border:"#b8960c",
      price:15000, priceLabel:"Rs.\u200b15,000/yr",
    ...
    { id:"ind_prem",  name:"Premium",  badge:"",           color:"#a78bfa", border:"#6d28d9",
      price:20000, priceLabel:"Rs.\u200b20,000/yr",
  ...
  company: [
    { id:"co_starter",name:"Starter",  badge:"",           color:"#06b6d4", border:"#0e7490",
      price:25000, priceLabel:"Rs.\u200b25,000/yr",
      desc:"Small company — app + annual filing support",
      features:["Company tax computation","Multi-bank & cash tracking","VAT monitoring","Annual return filing support","30-day free trial"],
      included:["Annual Return Filing"],
    },
    { id:"co_biz",    name:"Business", badge:"POPULAR",    color:"#b8960c", border:"#b8960c",
      price:40000, priceLabel:"Rs.\u200b40,000/yr",
    ...
    { id:"co_ent",    name:"Enterprise",badge:"",          color:"#f59e0b", border:"#92400e",
      price:60000, priceLabel:"Rs.\u200b60,000/yr",
      desc:"Full outsourced accounting, audit & compliance",
      features:["Everything in Business","Full bookkeeping (unlimited transactions)","Statutory audit certificate","Transfer pricing documentation","Unlimited employee payroll","Dedicated CA partner"],
...
const TAX_YEAR_CONFIG = {
  ...
  // ── 2022/2023 ── Two periods due to Inland Revenue Amendment Act No.45 of 2022
  // Period 1: 01 Apr 2022 – 31 Dec 2022 (old IRA 2017 rates, personal relief Rs.3,000,000)
  // Period 2: 01 Jan 2023 – 31 Mar 2023 (Amendment 45/2022, relief Rs.1,200,000)
  // For annual IIT computation the IRD requires filing two separate periods.
  // This app models the full year using Amendment 45/2022 rates (applicable to most of the filing).
  // Source: IRD Guideline SEC/2022-E/04 | IRA Amendment No.45 of 2022 (Gazette 2312/16)
  "2022/2023": {
    personalRelief:       1_200_000, // Amendment 45/2022 (Jan–Mar 2023 period; pre-Jan 2022: Rs.3,000,000)
    qpCap:                1_200_000,
    maxRefund:               45_000, // Prior to 2024/25 amendment
    filingThreshold:      1_200_000,
    exportRate:                0.15,
    aitRate:                   0.05, // WHT on interest: 5% (pre-2025)
    withholdingDividend:       0.15,
    apitMonthlyThreshold:   100_000, // Rs.100,000/month from Jan 2023
    act: "IRA No.24/2017 as amended by Amendment No.45 of 2022 (effective 01.01.2023)",
    periodNote: "Two-period year: pre-Jan 2023 relief Rs.3,000,000 → post-Jan 2023 relief Rs.1,200,000",
    // Annual IIT slabs on TAXABLE INCOME (after personal relief & QP deduction)
    // Source: APIT Table 01 for Y/A 2022/2023 (Jan–Mar 2023 period), IRD.gov.lk
    slabs: [
      { min:0,         max:  500_000, rate:0.06, label:"First Rs.500,000 @ 6%"       },
      { min:  500_000, max:  900_000, rate:0.18, label:"Next Rs.400,000 @ 18%"       },
      { min:  900_000, max:1_300_000, rate:0.24, label:"Next Rs.400,000 @ 24%"       },
      { min:1_300_000, max:1_700_000, rate:0.30, label:"Next Rs.400,000 @ 30%"       },
      { min:1_700_000, max:Infinity,  rate:0.36, label:"Balance above Rs.1.7M @ 36%" },
    ],
    apitTable01: [
      { maxMonthly:100_000,  formula:(p)=>0,              label:"≤ Rs.100,000 — No APIT" },
      { maxMonthly:141_667,  formula:(p)=>p*0.06-6000,    label:"Rs.100,001–141,667 @ 6%" },
      { maxMonthly:175_000,  formula:(p)=>p*0.18-23000,   label:"Rs.141,668–175,000 @ 18%" },
      { maxMonthly:208_333,  formula:(p)=>p*0.24-33500,   label:"Rs.175,001–208,333 @ 24%" },
      { maxMonthly:241_667,  formula:(p)=>p*0.30-50000,   label:"Rs.208,334–241,667 @ 30%" },
      { maxMonthly:Infinity, formula:(p)=>p*0.36-64500,   label:"Above Rs.241,667 @ 36%" },
    ],
    notes: [
      "⚠️ Two-period year: 01 Apr–31 Dec 2022 (old rates, Rs.3M relief) + 01 Jan–31 Mar 2023 (new, Rs.1.2M relief)",
      "Amendment Act No.45/2022 effective 01 January 2023 — APIT mandatory above Rs.100,000/month",
      "Annual IIT: File two period-returns separately (IRD Form AITR 2022/23)",
      "WHT on interest income: 5% (creditable against final IIT)",
      "Qualifying payments deduction: max Rs.1,200,000",
      "Export/foreign-sourced income: 15% flat rate",
      "Late filing penalty: 5% of tax + 1% per month (max 12 months)",
    ],
  },

  // ── 2023/2024 ── IRA Amendment 45/2022 full year
  // Personal relief Rs.1,200,000 applies throughout. Same slab structure as Jan–Mar 2023.
  // Source: IRD APIT Table 01 for Y/A 2023/2024 | Gazette No.2312/16
  "2023/2024": {
    ...
    aitRate:                   0.05,
    withholdingDividend:       0.15,
    apitMonthlyThreshold:   100_000,
    act: "IRA No.24/2017 as amended by Amendment No.45 of 2022 — Full Year Application",
    // Annual IIT slabs on TAXABLE INCOME (gross income − Rs.1,200,000 relief − QP deduction)
    // Derived from APIT Monthly Table 01 thresholds: Rs.100k, Rs.141.7k, Rs.175k, Rs.208.3k, Rs.241.7k
    slabs: [
      { min:0,         max:  500_000, rate:0.06, label:"First Rs.500,000 @ 6%"       },
      { min:  500_000, max:  900_000, rate:0.18, label:"Next Rs.400,000 @ 18%"       },
      { min:  900_000, max:1_300_000, rate:0.24, label:"Next Rs.400,000 @ 24%"       },
    ...
    notes: [
      "Personal relief Rs.1,200,000/year — mandatory AITR if gross income exceeds this",
      "Employer must deduct APIT from employees earning > Rs.100,000/month",
      "No 12% band — rates jump directly from 6% to 18% on taxable income",
      "WHT on FD interest: 5% (creditable) | Dividends: 15%",
      "Capital gains on listed shares: EXEMPT | Unlisted shares: assessable",
      "Qualifying payments (life insurance, EPF voluntary, approved charity): max Rs.1,200,000 deduction",
      "AITR filing deadline: 30 November 2024",
    ],
  },

  // ── 2024/2025 ── Same legal framework as 2023/2024
  // No new amendment changed the slabs or personal relief for this year.
  // WHT on interest remained at 5%. Source: CMA Sri Lanka Tax Rate Sheet Y/A 2024/2025
  "2024/2025": {
    personalRelief:       1_200_000,
    qpCap:                1_200_000,
    maxRefund:              180_000, // Increased to Rs.180,000 from 2024/25 per IRD Notice PN/IT/2025-01
    filingThreshold:      1_200_000,
    exportRate:                0.15,
    aitRate:                   0.05,
    withholdingDividend:       0.15,
    apitMonthlyThreshold:   100_000,
    act: "IRA No.24/2017 as amended (Budget 2024) — Y/A 2024/2025",
    slabs: [
      { min:0,         max:  500_000, rate:0.06, label:"First Rs.500,000 @ 6%"       },
      { min:  500_000, max:  900_000, rate:0.18, label:"Next Rs.400,000 @ 18%"       },
      { min:  900_000, max:1_300_000, rate:0.24, label:"Next Rs.400,000 @ 24%"       },
    ...
    notes: [
      "Personal relief Rs.1,200,000 unchanged — same APIT thresholds as 2023/24",
      "⚠️ Maximum tax refund increased to Rs.180,000/year (effective from this year — IRD Notice PN/IT/2025-01)",
      "Refund claims: submit within 30 months of last date of Y/A",
      "WHT on interest: 5% | Dividends: 15%",
      "AITR filing deadline: 30 November 2025",
      "IRD Ruling 2024/01: Stricter transfer pricing documentation for related-party transactions",
    ],
  },

  // ── 2025/2026 ── Inland Revenue (Amendment) Act No.02 of 2025 (certified 20 Mar 2025)
  // MAJOR CHANGES: Personal relief raised to Rs.1,800,000 (Rs.150,000/month) from 01 Apr 2025.
  // First slab widened from Rs.500,000 to Rs.1,000,000 @ 6%.
  // WHT on interest raised from 5% to 10%.
  // Export/foreign-sourced income now taxable at 15% (previously exempt).
  // Source: IRA Amendment No.2/2025 | IRD Notice PN/IT/2025-01 | IRD APIT Table 01 (2025/2026)
  "2025/2026": {
    personalRelief:       1_800_000, // Raised from Rs.1,200,000 — Act No.2/2025
    qpCap:                1_200_000,
    maxRefund:              180_000,
    filingThreshold:      1_800_000,
    exportRate:                0.15, // Now TAXABLE at 15% (previously EXEMPT) — Act No.2/2025
    aitRate:                   0.10, // WHT on interest RAISED from 5% to 10% — Act No.2/2025
    withholdingDividend:       0.15,
    apitMonthlyThreshold:   150_000, // Raised from Rs.100,000 to Rs.150,000/month
    act: "Inland Revenue (Amendment) Act No.02 of 2025 (certified 20 March 2025)",
    // Annual IIT slabs on TAXABLE INCOME (gross − Rs.1,800,000 relief − QP deduction)
    // Derived from IRD APIT Tax Table 01 for Y/A 2025/2026 (official IRD.gov.lk publication)
    // Monthly thresholds: Rs.150k, Rs.233.3k, Rs.275k, Rs.316.7k, Rs.358.3k
    slabs: [
    ...
    // APIT Monthly Table 01 (primary employment) — Y/A 2025/2026
    // Source: https://www.ird.gov.lk/en/publications/APIT_Tax_Tables/2025-2026/
    apitTable01: [
      { maxMonthly:150_000,  formula:(p)=>0,              label:"≤ Rs.150,000 — No APIT" },
      { maxMonthly:233_333,  formula:(p)=>p*0.06-9000,    label:"Rs.150,001–233,333 @ 6%" },
      { maxMonthly:275_000,  formula:(p)=>p*0.18-37000,   label:"Rs.233,334–275,000 @ 18%" },
      { maxMonthly:316_667,  formula:(p)=>p*0.24-53500,   label:"Rs.275,001–316,667 @ 24%" },
      { maxMonthly:358_333,  formula:(p)=>p*0.30-72500,   label:"Rs.316,668–358,333 @ 30%" },
      { maxMonthly:Infinity, formula:(p)=>p*0.36-94000,   label:"Above Rs.358,333 @ 36%" },
    ],
    notes: [
      "🆕 Personal relief raised to Rs.1,800,000 — no tax if monthly income ≤ Rs.150,000",
      "🆕 APIT mandatory for employees earning > Rs.150,000/month (raised from Rs.100,000)",
      "🆕 WHT on interest (AIT) raised from 5% to 10% from April 2025",
      "🆕 Service export income & foreign-sourced income now taxable at 15% flat (no longer exempt)",
      "First taxable slab widened to Rs.1,000,000 @ 6% (was Rs.500,000 @ 6% in prior years)",
      "No 12% band — rates: 6% → 18% → 24% → 30% → 36% on taxable income slabs",
      "Maximum tax refund: Rs.180,000/year — processed within 3 months",
      "Qualifying payments deduction unchanged: max Rs.1,200,000",
      "AITR filing deadline: 30 November 2026",
      "Source: IRA Amendment No.2/2025 certified 20 March 2025 | IRD Notice PN/IT/2025-01",
    ],
  },
...
const INCOME_CATS_SALARIED   = ["Employment","Invoice/Client Fees","Rental","Investment","Export/Foreign","Other Income"];
const INCOME_CATS_FREELANCER = ["Freelance/Contract","Invoice/Client Fees","Business Income","Rental","Investment","Export/Foreign","Other Income"];
...
const CHANGELOG = {
  "14.0.0": {
    date: "February 2026",
    highlights: [
      { icon:"💰", title:"Updated Pricing Plans", desc:"New Individual (Rs.8,000–20,000) & Company (Rs.25,000–60,000) annual plans" },
      { icon:"🧮", title:"APIT Calculator", desc:"Official Tax Table No.01 for 2023/24, 2024/25 & 2025/26 — monthly employer deduction" },
      { icon:"🏦", title:"3-Step Checkout", desc:"Bank transfer payment flow with Sampath Bank details & WhatsApp proof submission" },
      { icon:"📱", title:"PWA / Play Store Ready", desc:"Maskable icons, screenshots & manifest updated — installable from browser & Google Play" },
      { icon:"📒", title:"Cash Book Improvements", desc:"Running balance per row in red when negative — full HTML report with monthly summary" },
      { icon:"🔔", title:"Auto-Update Notifications", desc:"App notifies you instantly when a new version is available — tap to install in seconds" },
      { icon:"🏢", title:"Admin Payment Panel", desc:"Track subscriber payments, generate activation codes, 30-day renewal alerts" },
      { icon:"📋", title:"10 New Services", desc:"Transfer Pricing, Internal Audit, Business Valuation, Company Secretarial & more added to Services page" },
    ]
  }
};
...
function WhatsNewModal({ onClose }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        width:"100%", maxWidth:430, background:"#0a0f1a",
        ...
        <div style={{
          ...
          <div>
            <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:4}}>
              ...
              <span style={{fontSize:17, fontWeight:"800", color:"#f5d060"}}>
                What's New in v{APP_VERSION}
              </span>
            </div>
            <div style={{fontSize:11, color:"#b8960c"}}>{log.date} Update — FinTax LK</div>
          </div>
        ...
        <div style={{padding:"16px 14px 0"}}>
          {log.highlights.map((item,i)=>(
            <div key={i} style={{
              ...
              padding:"10px 0",
              borderBottom: i < log.highlights.length-1 ? "1px solid #0d1829" : "none",
            }}>
              <div style={{
        ...
        <div style={{padding:"16px 14px 0"}}>
          <button onClick={onClose} style={{
            ...
            background:"linear-gradient(135deg,#b8960c,#f5d060)",
            border:"none", borderRadius:12,
            color:"#111", fontWeight:"800", fontSize:14, cursor:"pointer",
          }}>
            ✅ Got it — Start Using v{APP_VERSION}
          </button>
...
// Called by service worker when a new version is waiting to activate
function useAppUpdateReady() {
  const [updateReady, setUpdateReady] = useState(false);
  useEffect(() => {
    const onReady = () => setUpdateReady(true);
    window.addEventListener("fintax_update_ready", onReady);
    // Already waiting before mount?
    if(window.__fintaxNewSW) setUpdateReady(true);
    return () => window.removeEventListener("fintax_update_ready", onReady);
  }, []);
  return updateReady;
}

function applyUpdate() {
  if(window.__fintaxNewSW){
    // Listen for the SW to take over, THEN reload (user-triggered only)
    navigator.serviceWorker.addEventListener("controllerchange", ()=>{
      window.location.reload();
    }, { once: true }); // { once: true } = fires only one time, not on every load
    window.__fintaxNewSW.postMessage({ type: "SKIP_WAITING" });
  } else {
    window.location.reload();
  }
}
...
function UpdateBanner() {
  ...
  return (
    <div style={{
      ...
      width:"100%", maxWidth:430, zIndex:99999,
      ...
      <div style={{
        ...
        display:"flex", alignItems:"center", justifyContent:"center",
        fontSize:18, border:"1.5px solid #10b981",
      }}>🚀</div>

      {/* Text */}
      <div style={{flex:1, minWidth:0}}>
        <div style={{fontSize:13, fontWeight:"800", color:"#34d399", lineHeight:1.2}}>
          FinTax LK v{APP_VERSION} is ready!
        </div>
        <div style={{fontSize:10, color:"#6ee7b7", marginTop:2, lineHeight:1.4}}>
          New features &amp; improvements available · Tap to install
        </div>
      </div>

      {/* Update button */}
      <button
        onClick={()=>{ setInstalling(true); applyUpdate(); }}
        disabled={installing}
        style={{
          flexShrink:0,
          padding:"8px 14px",
          background: installing ? "#1e293b" : "linear-gradient(135deg,#10b981,#34d399)",
          border:"none", borderRadius:8,
          color: installing ? "#475569" : "#022c22",
          fontWeight:"800", fontSize:12,
          cursor: installing ? "not-allowed" : "pointer",
          transition:"all 0.15s",
          whiteSpace:"nowrap",
        }}>
        {installing ? "⏳ Installing…" : "⬆️ Update Now"}
      </button>
      ...
      <button
        ...
        title="Dismiss — remind later">×</button>
    </div>
  );
}
// ─────────────────────────────────────────────────────────────────────────────
...
// ── PWA: inject manifest + register service worker ───────────────────────────
  useEffect(()=>{
    ...
    // Set page title
    // ── Hide splash screen — React has mounted successfully ──────────
    if(window.__hideAppLoading) window.__hideAppLoading();
    document.title="FinTax LK — Finance & Tax Manager";

    // ── Service Worker: register + watch for updates ─────────────────
    // NOTE: controllerchange → auto-reload is intentionally REMOVED
    // It caused an infinite reload loop (workbox clientsClaim fires on every load)
    // Updates are applied ONLY when user taps "Update Now" in the banner
    if("serviceWorker" in navigator){
      navigator.serviceWorker.register("/sw.js")
        .then(reg=>{
          // Check for a waiting SW immediately (e.g. after hard refresh)
          if(reg.waiting){
            window.__fintaxNewSW = reg.waiting;
            window.dispatchEvent(new CustomEvent("fintax_update_ready"));
          }

          // Watch for new SW installing while page is open
          reg.addEventListener("updatefound",()=>{
            const newWorker = reg.installing;
            if(!newWorker) return;
            newWorker.addEventListener("statechange",()=>{
              if(newWorker.state==="installed" && navigator.serviceWorker.controller){
                window.__fintaxNewSW = newWorker;
                window.dispatchEvent(new CustomEvent("fintax_update_ready"));
              }
            });
          });

          // Poll every 30 min for updates (for users who leave app open)
          const poll = setInterval(()=>{ reg.update().catch(()=>{}); }, 30*60*1000);
          // Store cleanup ref
          window.__fintaxSwPollCleanup = ()=> clearInterval(poll);
        }).catch(()=>{/* SW is optional — app works without it */});
    }

    return ()=>{ if(window.__fintaxSwPollCleanup) window.__fintaxSwPollCleanup(); };
  },[]);
  // ─────────────────────────────────────────────────────────────────────────────

  const [screen,setScreen]   = useState("boot");

  // ── What's New modal — shown once per app version (must be before any early returns) ──
  const [showWhatsNew, setShowWhatsNew] = useState(()=>{
    try {
      const seen = localStorage.getItem("fintaxlk_seen_version");
      return seen !== APP_VERSION;
    } catch(e) { return false; }
  });
  // Mark this version as seen (runs once when modal is first shown)
  useEffect(()=>{
    if(showWhatsNew) {
      try { localStorage.setItem("fintaxlk_seen_version", APP_VERSION); } catch(e){}
    }
  }, [showWhatsNew]);
  ...
  const [showTrialPopup,setShowTrialPopup]=useState(()=>{
    // Show popup when trial is <=3 days left and not already dismissed this session
    const dismissed=sessionStorage.getItem("trial_popup_dismissed");
    if(dismissed) return false;
    if(sub?.plan==="trial"){
      const left=Math.max(0,FREE_TRIAL_DAYS-daysDiff(today(),sub?.startDate||""));
      return left<=3;
    }
    return false;
  });
  const trialDaysUsed=sub?.plan==="trial"?daysDiff(today(),sub.startDate):0;
  const isLocked=(sub?.plan==="trial"&&trialDaysUsed>=FREE_TRIAL_DAYS)||(sub?.plan==="paid"&&daysDiff(today(),sub.expiryDate)>0);
  const isExpiringSoon=sub?.plan==="paid"&&daysDiff(today(),sub.expiryDate)>-30&&daysDiff(today(),sub.expiryDate)<=0;
  const isTrialWarning=sub?.plan==="trial"&&trialDaysLeft<=5&&trialDaysLeft>0;
  ...
  // ─── Trial Expiry Popup (shows when ≤3 days left) ────────────────────────────
  const TrialPopup=()=>!showTrialPopup?null:(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:16}}>
      <div style={{background:"linear-gradient(160deg,#0d0d00,#1a1500)",border:"2px solid #b8960c",borderRadius:18,padding:24,maxWidth:340,width:"100%",textAlign:"center"}}>
        <div style={{fontSize:36,marginBottom:8}}>⏰</div>
        <div style={{fontSize:18,fontWeight:"800",color:"#f5d060",marginBottom:6}}>
          {trialDaysLeft===0?"Free Trial Ended!":`${trialDaysLeft} Day${trialDaysLeft===1?"":"s"} Left`}
        </div>
        <div style={{fontSize:12,color:"#94a3b8",marginBottom:16,lineHeight:1.7}}>
          {trialDaysLeft===0
            ?"Your 30-day free trial has ended. Subscribe to keep all your data and continue using FinTax LK."
            :`Your free trial ends in ${trialDaysLeft} day${trialDaysLeft===1?"":"s"}. Subscribe now to continue without interruption.`}
        </div>
        <div style={{background:"#0d0d00",border:"1px solid #b8960c",borderRadius:10,padding:"10px 14px",marginBottom:14,textAlign:"left"}}>
          <div style={{fontSize:10,color:"#b8960c",fontWeight:"700",marginBottom:4}}>BEST VALUE — Individual Pro</div>
          <div style={{fontSize:20,fontWeight:"900",color:"#f5d060"}}>Rs.12,000<span style={{fontSize:11,color:"#64748b",fontWeight:"400"}}>/year</span></div>
          <div style={{fontSize:10,color:"#34d399",marginTop:4}}>🎁 Includes FREE AITR filing + 1hr consultation</div>
        </div>
        <button onClick={()=>{setShowTrialPopup(false);activatePaid();}} style={{width:"100%",padding:12,background:"linear-gradient(135deg,#b8960c,#f5d060)",border:"none",borderRadius:10,color:"#111",fontWeight:"800",fontSize:14,cursor:"pointer",marginBottom:10}}>
          💳 Subscribe Now — View Plans
        </button>
        <button onClick={()=>{setShowTrialPopup(false);sessionStorage.setItem("trial_popup_dismissed","1");}} style={{background:"transparent",border:"none",color:"#475569",fontSize:11,cursor:"pointer",textDecoration:"underline"}}>
          Remind me later
        </button>
      </div>
    </div>
  ...
  // Renewal popup — shown when subscription expires within 30 days
  const [showRenewalPopup,setShowRenewalPopup]=useState(()=>{
    const isPaid=sub?.plan==="paid";
    if(!isPaid) return false;
    const daysToExpiry=daysDiff(today(),sub.expiryDate);
    if(daysToExpiry>30||daysToExpiry<0) return false;
    return sessionStorage.getItem("renewal_popup_dismissed")!=="1";
  });
  const renewalDaysLeft=sub?.plan==="paid"?Math.max(0,daysDiff(today(),sub.expiryDate)):null;


  const RenewalPopup=()=>!showRenewalPopup?null:(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"linear-gradient(160deg,#0d0d00,#1a1500)",border:"2px solid #b8960c",borderRadius:20,padding:24,maxWidth:360,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontSize:32,marginBottom:8}}>⏰</div>
          <div style={{fontSize:18,fontWeight:"800",color:"#f5d060",marginBottom:6}}>
            Subscription Expires in {renewalDaysLeft} Day{renewalDaysLeft!==1?"s":""}
          </div>
          <div style={{fontSize:12,color:"#94a3b8",lineHeight:1.6,marginBottom:12}}>
            Your FinTax LK subscription expires on <strong style={{color:"#fbbf24"}}>{sub?.expiryDate}</strong>.<br/>
            Renew now to avoid losing access to all your tools and data.
          </div>
          <div style={{background:"#0a2a1a",border:"1px solid #064e3b",borderRadius:8,padding:"8px 12px",marginBottom:16,fontSize:11,color:"#34d399"}}>
            🔒 <strong>Your data is safe</strong> — all transactions, income records and reports remain stored on your device even after expiry.
          </div>
        </div>
        <button onClick={()=>{setShowRenewalPopup(false);activatePaid();}}
          style={{width:"100%",padding:14,background:"linear-gradient(135deg,#b8960c,#f5d060)",border:"none",borderRadius:12,color:"#111",fontWeight:"800",fontSize:14,cursor:"pointer",marginBottom:10}}>
          💳 Renew Now — Same Plan & Price
        </button>
        <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${encodeURIComponent("Hi GDP Consultants, my FinTax LK subscription expires on "+sub?.expiryDate+". I'd like to renew. My name: "+(profile?.name||"")+" mobile: "+(profile?.mobile||""))}`}
          target="_blank"
          style={{display:"block",marginBottom:8,background:"#064e3b",color:"#6ee7b7",border:"1px solid #10b981",
            borderRadius:10,padding:12,fontWeight:"700",fontSize:12,textDecoration:"none",textAlign:"center"}}>
          💬 WhatsApp Us to Renew
        </a>
        <button onClick={()=>{setShowRenewalPopup(false);sessionStorage.setItem("renewal_popup_dismissed","1");}}
          style={{background:"transparent",border:"none",color:"#475569",fontSize:11,cursor:"pointer",textDecoration:"underline",width:"100%"}}>
          Remind me later
        </button>
      </div>
    </div>
  ...
  return(
    <div style={S.phone}>
      ...
      {/* ── Trial Warning Popup (≤5 days left) ── */}
      {isTrialWarning&&(
        <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,zIndex:999,padding:"0 12px",boxSizing:"border-box"}}>
          <div style={{background:"linear-gradient(135deg,#1a0e00,#2a1800)",border:"2px solid #b8960c",borderRadius:"0 0 16px 16px",padding:"10px 14px",display:"flex",gap:10,alignItems:"center"}}>
            <span style={{fontSize:22}}>⏰</span>
            <div style={{flex:1}}>
              <div style={{fontSize:12,fontWeight:"800",color:"#f5d060"}}>Trial ends in {trialDaysLeft} day{trialDaysLeft!==1?"s":""}!</div>
              <div style={{fontSize:10,color:"#94a3b8"}}>Subscribe now to keep your data & features</div>
            </div>
            <button onClick={()=>setTab("services")}
              style={{background:"linear-gradient(135deg,#b8960c,#f5d060)",border:"none",borderRadius:8,
                padding:"6px 10px",color:"#111",fontWeight:"800",fontSize:11,cursor:"pointer",flexShrink:0}}>
              Subscribe →
            </button>
          </div>
        </div>
      ...
      {/* ── Renewal Warning (≤30 days to expiry) ── */}
      {isExpiringSoon&&(
        <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,zIndex:999,padding:"0 12px",boxSizing:"border-box"}}>
          <div style={{background:"linear-gradient(135deg,#1a0020,#2a0035)",border:"2px solid #a78bfa",borderRadius:"0 0 16px 16px",padding:"10px 14px",display:"flex",gap:10,alignItems:"center"}}>
            <span style={{fontSize:22}}>🔄</span>
            <div style={{flex:1}}>
              <div style={{fontSize:12,fontWeight:"800",color:"#c4b5fd"}}>Subscription renews soon</div>
              <div style={{fontSize:10,color:"#94a3b8"}}>Expires: {sub?.expiryDate} — renew to avoid interruption</div>
            </div>
            <button onClick={activatePaid}
              style={{background:"linear-gradient(135deg,#7c3aed,#a78bfa)",border:"none",borderRadius:8,
                padding:"6px 10px",color:"#fff",fontWeight:"800",fontSize:11,cursor:"pointer",flexShrink:0}}>
              Renew →
            </button>
          </div>
        </div>
...
function Paywall({onPay,onActivateCode,reason,profile,lang,setLang}){
  const [step,setStep]=useState(1);            // 1=type  2=plan  3=checkout
  ...
  const handleCodeSubmit=async()=>{
    ...
    if(result.ok){setCodeStatus("ok");setCodeMsg(T(lang,"activated"));}
    else{setCodeStatus("error");setCodeMsg("❌ "+result.reason);}
  };
  ...
  const plans=clientType?PLANS[clientType]:[];

  const waProof=selectedPlan?encodeURIComponent(
    `Hi GDP Consultants! I have deposited Rs.${(selectedPlan.price).toLocaleString()} for FinTax LK ${clientType==="individual"?"Individual":"Company"} - ${selectedPlan.name} Plan.\n`+
    `Name: ${profile?.name||"[Your Name]"}\nMobile: ${profile?.mobile||"[Your Number]"}\n`+
    `Please send my activation code. Payment slip attached.`
  ):"";

  const stepDot=(n)=>(
    <div style={{width:n===step?22:8,height:8,borderRadius:4,
      background:n<step?"#10b981":n===step?"#b8960c":"#1e293b",transition:"all 0.25s"}}/>
  );

  return(
    <div style={{...S.phone,overflowY:"auto",padding:"16px 14px 30px",justifyContent:"flex-start",alignItems:"center",
      background:"linear-gradient(160deg,#060d1a 0%,#0a0800 100%)"}}>

      <div style={{position:"absolute",top:10,right:10,zIndex:10}}>
        <LangSelector lang={lang} setLang={setLang}/>
      </div>

      <img src={LOGO_B64} alt="" style={{width:56,height:56,borderRadius:"50%",border:"2px solid #b8960c",marginBottom:8,marginTop:22}}/>
      <div style={{fontSize:17,fontWeight:"800",color:"#f1f5f9",textAlign:"center",marginBottom:2}}>
        {reason==="trial"?T(lang,"trial_ended"):T(lang,"sub_expired")}
      </div>
      <div style={{fontSize:11,color:"#64748b",textAlign:"center",marginBottom:10,maxWidth:300,lineHeight:1.5}}>
      ...
      <div style={{width:"100%",boxSizing:"border-box",marginBottom:8}}>
        <button onClick={()=>{setShowCode(v=>!v);setCodeStatus(null);setCodeMsg("");setShowTransfer(false);}}
          style={{width:"100%",padding:"10px",background:showCode?"#1a1200":"linear-gradient(135deg,#064e3b,#065f46)",
            border:"2px solid "+(showCode?"#b8960c":"#10b981"),borderRadius:10,
            color:showCode?"#b8960c":"#6ee7b7",fontWeight:"800",fontSize:13,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          🔑 {showCode?T(lang,"hide_code"):T(lang,"have_code")}
        </button>
        {showCode&&(
          <div style={{background:"#0a1a14",border:"2px solid #10b981",borderTop:"none",borderRadius:"0 0 10px 10px",padding:"14px"}}>
            <div style={{fontSize:10,color:"#64748b",marginBottom:8}}>{T(lang,"code_hint")}</div>
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <input value={codeInput} onChange={e=>setCodeInput(e.target.value.toUpperCase())}
                onKeyDown={e=>{if(e.key==="Enter")handleCodeSubmit();}}
                placeholder="GDP-XXXX-XXXX" maxLength={12}
                style={{...S.inp,flex:1,fontFamily:"monospace",fontSize:14,fontWeight:"700",
                  letterSpacing:2,textAlign:"center",marginBottom:0,background:"#0a2a1a",borderColor:"#10b981"}}/>
              <button onClick={handleCodeSubmit} disabled={!codeInput.trim()||codeStatus==="checking"||codeStatus==="ok"}
                style={{background:codeStatus==="checking"?"#334155":"#10b981",color:"#fff",border:"none",
                  borderRadius:8,padding:"0 14px",fontWeight:"800",fontSize:12,cursor:"pointer",flexShrink:0}}>
                {codeStatus==="checking"?T(lang,"checking"):T(lang,"activate")}
              </button>
            </div>
            {codeMsg&&(
              <div style={{fontSize:12,fontWeight:"700",color:codeStatus==="ok"?"#34d399":"#f87171",
                padding:"8px 10px",background:codeStatus==="ok"?"#0a2a1a":"#1a0000",borderRadius:8,
                border:"1px solid "+(codeStatus==="ok"?"#064e3b":"#7f1d1d")}}>
                {codeMsg}
              </div>
            )}
      ...
      {/* Device Transfer */}
      <div style={{width:"100%",boxSizing:"border-box",marginBottom:12}}>
        <button onClick={()=>{setShowTransfer(v=>!v);setShowCode(false);}}
          style={{width:"100%",padding:"8px",background:"transparent",border:"1px solid #1e293b",
            borderRadius:8,color:"#475569",fontWeight:"600",fontSize:11,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          📱 {T(lang,"device_transfer")}
        </button>
        {showTransfer&&(
          <div style={{background:"#0a1525",border:"1px solid #1e3a5f",borderTop:"none",borderRadius:"0 0 8px 8px",padding:"12px"}}>
            <div style={{fontSize:11,color:"#64748b",lineHeight:1.7,marginBottom:8}}>{T(lang,"device_transfer_info")}</div>
            <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=Hello GDP Consultants, I need to transfer my FinTax LK subscription to a new device. My name is ${encodeURIComponent(profile?.name||"")} and my registered mobile is ${profile?.mobile||""}.`}
              target="_blank"
              style={{display:"block",background:"#064e3b",color:"#6ee7b7",border:"1px solid #10b981",
                borderRadius:8,padding:"8px",fontWeight:"700",fontSize:12,textDecoration:"none",textAlign:"center"}}>
      ...
      {step===1&&(
        <div style={{width:"100%"}}>
          <div style={{textAlign:"center",fontSize:14,fontWeight:"800",color:"#f1f5f9",marginBottom:4}}>
            Select Your Plan Type
          </div>
          <div style={{textAlign:"center",fontSize:10,color:"#64748b",marginBottom:14}}>
            We offer separate plans for individuals and companies
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            {[
              {type:"individual",icon:"👤",label:"Individual",sub:"Salaried / Freelancer",
               color:"#b8960c",border:"#b8960c",bg:"linear-gradient(135deg,#1a1200,#2a1c00)",from:"From Rs.8,000/yr"},
              {type:"company",icon:"🏢",label:"Company",sub:"Business / Corporate",
               color:"#06b6d4",border:"#0e7490",bg:"linear-gradient(135deg,#01101a,#011a28)",from:"From Rs.25,000/yr"},
            ].map(opt=>(
              <div key={opt.type} onClick={()=>{setClientType(opt.type);setSelectedPlan(null);setStep(2);}}
                style={{background:opt.bg,border:`2px solid ${opt.border}`,borderRadius:14,padding:"18px 10px",
                  textAlign:"center",cursor:"pointer"}}>
                <div style={{fontSize:30,marginBottom:8}}>{opt.icon}</div>
                <div style={{fontSize:13,fontWeight:"800",color:opt.color,marginBottom:4}}>{opt.label}</div>
                <div style={{fontSize:10,color:"#64748b",marginBottom:8}}>{opt.sub}</div>
                <div style={{fontSize:11,fontWeight:"700",color:"#f5d060"}}>{opt.from}</div>
                <div style={{marginTop:10,fontSize:10,color:opt.color,fontWeight:"700"}}>Select →</div>
              </div>
            ))}
          </div>
          <div style={{textAlign:"center",fontSize:10,color:"#334155"}}>
            30-day free trial included · Activate with code after payment
          </div>
        </div>
      )}
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            <button onClick={()=>setStep(1)}
              style={{background:"#1e293b",border:"1px solid #334155",color:"#94a3b8",
                borderRadius:8,padding:"5px 10px",fontSize:11,cursor:"pointer",fontWeight:"700"}}>
              ← Back
            </button>
            <div>
              <div style={{fontSize:13,fontWeight:"800",color:"#f1f5f9"}}>
                {clientType==="individual"?"👤 Individual Plans":"🏢 Company Plans"}
              </div>
            </div>
          </div>
          {plans.map(p=>(
            <div key={p.id} onClick={()=>{setSelectedPlan(p);setStep(3);}}
              style={{background:"#0a0f1a",border:`2px solid ${p.border}`,borderRadius:12,
                padding:"14px",marginBottom:10,cursor:"pointer",position:"relative",
                boxShadow:p.badge?"0 0 14px rgba(184,150,12,0.15)":"none"}}>
              {p.badge&&(
                <div style={{position:"absolute",top:-1,right:10,background:p.color,color:"#111",
                  fontSize:8,fontWeight:"800",padding:"2px 8px",borderRadius:"0 0 6px 6px",letterSpacing:0.5}}>
                  ⭐ {p.badge}
                </div>
              )}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                ...
                <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
                  <div style={{fontSize:18,fontWeight:"900",color:"#f5d060"}}>Rs.{p.price.toLocaleString()}</div>
                  <div style={{fontSize:9,color:"#64748b"}}>/year</div>
                </div>
              </div>
              <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:p.included.length>0?8:0}}>
                {p.features.map(f=>(
                  <div key={f} style={{fontSize:9,color:"#94a3b8",background:"#0d1829",
                    borderRadius:4,padding:"2px 5px",border:"1px solid #1e293b"}}>
                    ✓ {f}
                  </div>
                ))}
              </div>
              {p.included.length>0&&(
                <div style={{background:`${p.color}18`,border:`1px solid ${p.color}40`,
                  borderRadius:6,padding:"5px 8px",display:"flex",gap:6,flexWrap:"wrap"}}>
                  {p.included.map(inc=>(
                    <span key={inc} style={{fontSize:9,color:p.color,fontWeight:"700"}}>🎁 {inc}</span>
                  ))}
                </div>
              )}
              <div style={{marginTop:8,textAlign:"right",fontSize:11,fontWeight:"700",color:p.color}}>
                Choose this plan →
              </div>
            </div>
          ))}
      ...
      {step===3&&selectedPlan&&(
        <div style={{width:"100%"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            ...
            <div style={{fontSize:13,fontWeight:"800",color:"#f1f5f9"}}>Payment Details</div>
          </div>

          {/* Order Summary */}
          <div style={{background:"linear-gradient(135deg,#0d0d00,#1a1500)",
            border:`2px solid ${selectedPlan.border}`,borderRadius:12,padding:14,marginBottom:12}}>
            <div style={{fontSize:10,color:"#64748b",fontWeight:"700",letterSpacing:1,marginBottom:8}}>ORDER SUMMARY</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <div>
                <div style={{fontSize:14,fontWeight:"800",color:selectedPlan.color}}>
                  {clientType==="individual"?"👤":"🏢"} {selectedPlan.name} Plan
                </div>
                <div style={{fontSize:10,color:"#64748b"}}>{selectedPlan.desc}</div>
              </div>
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:20,fontWeight:"900",color:"#f5d060"}}>Rs.{selectedPlan.price.toLocaleString()}</div>
                <div style={{fontSize:9,color:"#64748b"}}>/year</div>
              </div>
            </div>
            {selectedPlan.included.length>0&&(
              <div style={{borderTop:"1px solid #1e293b",paddingTop:6,marginTop:6}}>
                {selectedPlan.included.map(inc=>(
                  <div key={inc} style={{fontSize:10,color:"#34d399",padding:"1px 0"}}>🎁 Includes: {inc}</div>
                ))}
              </div>
            )}
            <div style={{borderTop:"1px solid #1e293b",marginTop:8,paddingTop:8,
              display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{fontSize:12,fontWeight:"700",color:"#f1f5f9"}}>TOTAL DUE</div>
              <div style={{fontSize:22,fontWeight:"900",color:"#f5d060"}}>Rs.{selectedPlan.price.toLocaleString()}</div>
            </div>
          </div>

          {/* Bank Transfer Details */}
          {BANK_ACCOUNTS.map((acc,i)=>(
            <div key={i} style={{background:"#060d1a",border:"1px solid #1e3a5f",borderRadius:12,padding:14,marginBottom:10}}>
              <div style={{fontSize:12,fontWeight:"800",color:"#93c5fd",marginBottom:10}}>
                🏦 Bank Transfer — {acc.bank}
              </div>
              <div style={{background:"#0a1525",borderRadius:10,padding:"10px 12px",marginBottom:10}}>
                {[["Account Name",acc.acName],["Account No.",acc.acNo],["Bank",acc.bank],["Branch",acc.branch],["Address",acc.address],["SWIFT/BIC",acc.swift]].map(([lbl,val])=>(
                  <div key={lbl} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:6}}>
                    <span style={{fontSize:9,color:"#475569",fontWeight:"600",minWidth:80,paddingTop:2}}>{lbl}</span>
                    <span style={{fontSize:11,color:"#e2e8f0",fontWeight:"700",fontFamily:lbl.includes("No")||lbl.includes("SWIFT")?"monospace":"inherit",flex:1}}>{val}</span>
                    <button onClick={()=>copyField(val)}
                      style={{background:copied===val?"#064e3b":"#1e293b",color:copied===val?"#34d399":"#64748b",
                        border:"none",borderRadius:4,padding:"2px 6px",fontSize:9,cursor:"pointer",fontWeight:"700",flexShrink:0,minWidth:36}}>
                      {copied===val?"✓ OK":"Copy"}
                    </button>
                  </div>
                ))}
              ...
              <div style={{background:"#0a2a1a",border:"1px solid #064e3b",borderRadius:8,padding:"10px 12px"}}>
                <div style={{fontSize:11,fontWeight:"800",color:"#34d399",marginBottom:6}}>
                  ✅ After Depositing — 3 Easy Steps
                </div>
                <div style={{fontSize:10,color:"#94a3b8",lineHeight:1.8}}>
                  <div>1️⃣ Deposit <strong style={{color:"#f5d060"}}>Rs.{selectedPlan.price.toLocaleString()}</strong> to the account above</div>
                  <div>2️⃣ Take a photo/screenshot of your deposit slip or bank confirmation</div>
                  <div>3️⃣ WhatsApp the proof to <strong style={{color:"#34d399"}}>{APP_WHATSAPP}</strong> with your name &amp; mobile</div>
                  <div style={{marginTop:4,color:"#b8960c",fontWeight:"700"}}>🕐 We send your activation code within 2 working hours</div>
                </div>
              </div>

              <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${waProof}`}
                target="_blank"
                style={{display:"block",marginTop:10,background:"linear-gradient(135deg,#064e3b,#065f46)",
                  border:"1px solid #10b981",borderRadius:8,padding:"12px",
                  color:"#6ee7b7",fontWeight:"800",fontSize:13,textDecoration:"none",textAlign:"center"}}>
                📲 WhatsApp Payment Proof Now
              </a>
            </div>
          ))}

          <div style={{background:"linear-gradient(135deg,#1a1200,#2a1c00)",
            border:"1px solid #b8960c",borderRadius:10,padding:"10px 14px",
            display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontSize:10,color:"#b8960c",fontWeight:"700"}}>Use as deposit reference:</div>
            <div style={{fontSize:11,fontWeight:"800",color:"#f5d060",textAlign:"right"}}>
              FinTax {selectedPlan.name}<br/>
              <span style={{fontSize:13}}>Rs.{selectedPlan.price.toLocaleString()}</span>
            </div>
          </div>

          <div style={{fontSize:10,color:"#334155",textAlign:"center",lineHeight:1.7,marginBottom:6}}>
            Working hours: Mon–Fri 9am–6pm · Sat 9am–1pm<br/>
            WhatsApp or call: <span style={{color:"#94a3b8"}}>{APP_WHATSAPP}</span>
          </div>
          <ContactLinks small/>
        </div>
      )}
...
function AdminPanel({onExit,lang,setLang}){
  ...
  const [view,setView]=useState("proofs"); // proofs | codes | all | reg | pay | pin
  const [newCode,setNewCode]=useState(null);
  const [noteInput,setNoteInput]=useState("");
  const [planNote,setPlanNote]=useState("");
  const [amtInput,setAmtInput]=useState(String(SUBSCRIPTION_PRICE));
  ...
  const handlePinChange=async()=>{
    ...
    if(pinNew!==pinConfirm){setPinMsg({type:"error",text:"PINs don't match."});setPinConfirm("");return;}
  ...
  // Payment proof submissions — logged when user taps "WhatsApp Payment Proof"
  // We track these by storing a local log entry when admin reviews
  const proofReqs=log.filter(n=>n.type==="PAYMENT_PROOF_SENT"||n.type==="REGISTRATION");
  ...
  const handleGenerate=async()=>{
    setGenerating(true);
    const code=genCode();
    const amt=parseInt(amtInput)||SUBSCRIPTION_PRICE;
    const meta={note:(noteInput.trim()||"Admin activation")+(planNote?" | Plan: "+planNote:""),amount:amt};
    await saveCode(code,meta);
    // Log the activation
    await notifyAdmin("PAYMENT_RECEIVED",{name:noteInput.trim()||"Admin",mobile:"",amount:amt,plan:planNote||"Manual",paidAt:new Date().toISOString(),activatedVia:"admin-code",code});
    setNewCode(code);
    setNoteInput("");setPlanNote("");setAmtInput(String(SUBSCRIPTION_PRICE));
    await refreshData();
    setGenerating(false);
  };
  ...
  // Quick-activate: generate code + pre-fill WhatsApp with subscriber info
  const quickActivate=(reg)=>{
    setPlanNote(`Activation for ${reg.data?.name||""}`);
    setNoteInput(reg.data?.name||"");
    setView("codes");
  };
  ...
  return(
    <div style={S.phone}>
      <TrialPopup/>
      <RenewalPopup/>
      <div style={{background:"linear-gradient(135deg,#0a0020,#1a0040)",padding:"12px 14px",borderBottom:"1px solid #7c3aed",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:15,fontWeight:"800",color:"#c4b5fd"}}>🔐 Admin Dashboard</div>
            <div style={{fontSize:10,color:"#7c3aed"}}>GDP Consultants — {APP_NAME}</div>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <LangSelector lang={lang} setLang={setLang} dark/>
            <button style={{background:"#1e0050",border:"1px solid #7c3aed",color:"#a78bfa",borderRadius:8,padding:"5px 10px",fontSize:11,cursor:"pointer"}} onClick={onExit}>✕ Exit</button>
          </div>
        </div>
      </div>

      <div style={{padding:"12px",overflowY:"auto",flex:1,paddingBottom:24}}>

        {/* KPI Row */}
        <div style={S.cardRow}>
          <div style={{...S.card,flex:1,textAlign:"center",border:"1px solid #10b981",padding:"8px 4px"}}>
            <div style={{fontSize:18,fontWeight:"800",color:"#34d399"}}>{registrations.length}</div>
            <div style={{fontSize:9,color:"#64748b"}}>Users</div>
          </div>
          <div style={{...S.card,flex:1,textAlign:"center",border:"1px solid #f59e0b",padding:"8px 4px"}}>
            <div style={{fontSize:18,fontWeight:"800",color:"#fbbf24"}}>{payments.length}</div>
            <div style={{fontSize:9,color:"#64748b"}}>Paid</div>
          </div>
          <div style={{...S.card,flex:1,textAlign:"center",border:"1px solid #a78bfa",padding:"8px 4px"}}>
            <div style={{fontSize:14,fontWeight:"800",color:"#c4b5fd"}}>{unusedCodes.length}</div>
            <div style={{fontSize:9,color:"#64748b"}}>Codes</div>
          </div>
          <div style={{...S.card,flex:1,textAlign:"center",border:"1px solid #b8960c",padding:"8px 4px"}}>
            <div style={{fontSize:12,fontWeight:"800",color:"#f5d060"}}>{fmtC(totalRevenue)}</div>
        ...
        <div style={{display:"flex",gap:4,marginBottom:12,flexWrap:"wrap"}}>
          {[
            ...
            <button key={v} style={{...S.tBtn,flexShrink:0,...(view===v?{background:"#1a0050",borderColor:"#7c3aed",color:"#c4b5fd"}:{})}} onClick={()=>{setView(v);setPinMsg({type:"",text:""});}}>
              {l}{v==="proofs"&&registrations.length>0&&<span style={{background:"#ef4444",color:"#fff",borderRadius:"50%",width:14,height:14,fontSize:8,display:"inline-flex",alignItems:"center",justifyContent:"center",marginLeft:4,fontWeight:"800"}}>{registrations.filter(r=>!r.data?.activated).length}</span>}
            </button>
          ))}
          <button style={{...S.tBtn,marginLeft:"auto",fontSize:10}} onClick={refreshData}>🔄</button>
        </div>

        {/* ═══ PAYMENT REQUESTS TAB ═══ */}
        {view==="proofs"&&(
          <>
            <div style={{...S.card,border:"1px solid #06b6d4",background:"linear-gradient(160deg,#01101a,#011a28)",marginBottom:12}}>
              <div style={{fontSize:12,fontWeight:"800",color:"#06b6d4",marginBottom:8}}>📨 Subscriber Activation Requests</div>
              <div style={{fontSize:10,color:"#64748b",lineHeight:1.6}}>
                When a subscriber sends WhatsApp payment proof, verify & generate their code below.<br/>
                Bank: <strong style={{color:"#93c5fd"}}>{BANK_ACCOUNTS[0].bank}</strong> · Acc: <strong style={{color:"#f5d060",fontFamily:"monospace"}}>{BANK_ACCOUNTS[0].acNo}</strong>
              </div>
            </div>

            {registrations.length===0&&<Empty icon="📭" text="No subscriber registrations yet"/>}

            {[...registrations].reverse().map(reg=>(
              <div key={reg.id} style={{...S.card,border:"1px solid #1e293b",marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:"800",color:"#e2e8f0"}}>{reg.data?.name||"Unknown"}</div>
                    <div style={{fontSize:10,color:"#64748b"}}>{reg.data?.mobile||""} {reg.data?.tin?" · TIN: "+reg.data.tin:""}</div>
                    <div style={{fontSize:10,color:"#64748b"}}>{reg.data?.employmentType||""} · {reg.data?.city||""}</div>
                  </div>
                  <div style={{fontSize:9,color:"#475569",textAlign:"right"}}>
                    {new Date(reg.timestamp).toLocaleString("en-LK")}<br/>
                    {reg.data?.activated&&<span style={{color:"#10b981",fontWeight:"700"}}>✅ Activated</span>}
                    {!reg.data?.activated&&<span style={{color:"#f59e0b",fontWeight:"700"}}>⏳ Pending</span>}
                  </div>
                </div>

                {!reg.data?.activated&&(
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>quickActivate(reg)}
                      style={{flex:1,padding:"8px",background:"linear-gradient(135deg,#7c3aed,#a78bfa)",border:"none",borderRadius:8,color:"#fff",fontWeight:"800",fontSize:11,cursor:"pointer"}}>
                      🔑 Generate Code for This User
                    </button>
                    <a href={`https://wa.me/94${(reg.data?.mobile||APP_WHATSAPP).replace(/^0/,"")}?text=${encodeURIComponent("Hi "+reg.data?.name+", thanks for registering on FinTax LK! Please send your payment proof to activate.")}`}
                      target="_blank"
                      style={{padding:"8px 10px",background:"#064e3b",border:"1px solid #10b981",borderRadius:8,color:"#6ee7b7",fontWeight:"700",fontSize:11,textDecoration:"none",display:"flex",alignItems:"center"}}>
                      💬
                    </a>
                  </div>
                )}
                ...
                {reg.data?.activated&&(
                  <div style={{fontSize:10,color:"#34d399",padding:"6px 8px",background:"#0a2a1a",borderRadius:6,border:"1px solid #064e3b"}}>
                    ✅ Activated on {reg.data.activatedAt?new Date(reg.data.activatedAt).toLocaleDateString("en-LK"):"—"}
                  </div>
                )}
              </div>
            ))}
            ...
            {/* RENEWAL ALERTS */}
            {usedCodes.filter(c=>{
              const expiry=c.usedAt?new Date(new Date(c.usedAt).getTime()+365*86400000):null;
              const daysLeft=expiry?Math.ceil((expiry-Date.now())/86400000):null;
              return daysLeft!==null&&daysLeft<=30&&daysLeft>0;
            }).length>0&&(
              <div style={{...S.card,border:"2px solid #f59e0b",background:"linear-gradient(135deg,#1a0e00,#2a1500)"}}>
                <div style={{fontSize:12,fontWeight:"800",color:"#fbbf24",marginBottom:8}}>⏰ Renewals Due Soon</div>
                {usedCodes.filter(c=>{
                  const expiry=c.usedAt?new Date(new Date(c.usedAt).getTime()+365*86400000):null;
                  const daysLeft=expiry?Math.ceil((expiry-Date.now())/86400000):null;
                  return daysLeft!==null&&daysLeft<=30&&daysLeft>0;
                }).map(c=>{
                  const expiry=new Date(new Date(c.usedAt).getTime()+365*86400000);
                  const daysLeft=Math.ceil((expiry-Date.now())/86400000);
                  return(
                    <div key={c.code} style={{padding:"8px 0",borderBottom:"1px solid #1e293b",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div>
                        <div style={{fontSize:12,fontWeight:"700",color:"#e2e8f0"}}>{c.usedBy?.name||c.note||"Unknown"}</div>
                        <div style={{fontSize:10,color:"#64748b"}}>Code: {c.code} · Expires: {expiry.toLocaleDateString("en-LK")}</div>
                      </div>
                      <div style={{background:daysLeft<=7?"#7f1d1d":"#1a0e00",border:`1px solid ${daysLeft<=7?"#ef4444":"#f59e0b"}`,
                        borderRadius:8,padding:"4px 8px",textAlign:"center"}}>
                        <div style={{fontSize:13,fontWeight:"800",color:daysLeft<=7?"#f87171":"#fbbf24"}}>{daysLeft}d</div>
                        <div style={{fontSize:8,color:"#64748b"}}>left</div>
                      </div>
                    </div>
                  );
                })}
        ...
        {view==="codes"&&(
          <>
            <div style={{...S.card,border:"2px solid #7c3aed",background:"linear-gradient(160deg,#0a0020,#1a0040)",marginBottom:14}}>
              <div style={{fontSize:13,fontWeight:"800",color:"#c4b5fd",marginBottom:10}}>🔑 Generate Activation Code</div>
              <div style={{fontSize:10,color:"#7c3aed",marginBottom:10,lineHeight:1.6}}>
                Generate after verifying bank payment. Send code to subscriber via WhatsApp.
              </div>
              <div style={{marginBottom:8}}>
                <label style={{...S.lbl,color:"#a78bfa"}}>Customer Name &amp; Mobile</label>
                <input value={noteInput} onChange={e=>setNoteInput(e.target.value)} placeholder="e.g. Kamal Perera — 0771234567" style={{...S.inp,background:"#0a0020",borderColor:"#4c1d95",color:"#e2e8f0"}}/>
              </div>
              <div style={{marginBottom:8}}>
                <label style={{...S.lbl,color:"#a78bfa"}}>Plan Selected</label>
                <select value={planNote} onChange={e=>setPlanNote(e.target.value)}
                  style={{...S.inp,background:"#0a0020",borderColor:"#4c1d95",color:"#e2e8f0"}}>
                  <option value="">— Select Plan —</option>
                  {["Individual Basic Rs.4,800","Individual Pro Rs.12,000","Individual Premium Rs.24,000",
                    "Company Starter Rs.18,000","Company Business Rs.36,000","Company Enterprise Rs.60,000",
                    "Manual/Custom"].map(p=><option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <div style={{marginBottom:10}}>
                <label style={{...S.lbl,color:"#a78bfa"}}>Amount Received (Rs.)</label>
                <input value={amtInput} onChange={e=>setAmtInput(e.target.value)} type="number" style={{...S.inp,background:"#0a0020",borderColor:"#4c1d95",color:"#e2e8f0"}}/>
              </div>
              <button onClick={handleGenerate} disabled={generating}
                style={{width:"100%",padding:"12px",background:generating?"#1e0050":"linear-gradient(135deg,#7c3aed,#a78bfa)",
              ...
              {newCode&&(
                <div style={{marginTop:12,background:"#050010",border:"2px solid #a78bfa",borderRadius:12,padding:"14px",textAlign:"center"}}>
                  <div style={{fontSize:10,color:"#a78bfa",fontWeight:"700",marginBottom:6,letterSpacing:1}}>✅ CODE READY — SEND TO SUBSCRIBER</div>
                  <div style={{fontSize:26,fontWeight:"900",color:"#f5d060",letterSpacing:4,fontFamily:"monospace",marginBottom:10}}>{newCode}</div>
                  <button onClick={()=>copyToClipboard(newCode)}
                    style={{background:"#7c3aed",color:"#fff",border:"none",borderRadius:8,padding:"8px 18px",
                      fontWeight:"700",fontSize:12,cursor:"pointer",marginBottom:8,width:"100%"}}>
                    📋 {copyMsg||"Copy Code"}
                  </button>
                  <a href={`https://wa.me/94?text=Dear ${encodeURIComponent(noteInput||"Customer")}, your FinTax LK activation code is: ${newCode}%0APlan: ${encodeURIComponent(planNote||"")}%0AEnter in the app to activate 1-year access.%0AThank you — GDP Consultants`}
                    target="_blank"
                    style={{display:"block",background:"#064e3b",color:"#6ee7b7",border:"1px solid #10b981",
                      borderRadius:8,padding:"8px 18px",fontWeight:"700",fontSize:12,textDecoration:"none",marginBottom:4}}>
                    💬 Send via WhatsApp
                  </a>
                  <div style={{fontSize:9,color:"#475569",marginTop:4}}>Enter code in app → 1-year access granted automatically</div>
                </div>
              )}
            </div>
            ...
            {unusedCodes.length>0&&(
              <div style={S.card}>
                <div style={{...S.cardTitle,color:"#a78bfa"}}>⏳ Active Codes — {unusedCodes.length}</div>
                {unusedCodes.map(c=>(
                  <div key={c.code} style={{padding:"8px 0",borderBottom:"1px solid #1e293b"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:15,fontWeight:"900",color:"#f5d060",fontFamily:"monospace",letterSpacing:2}}>{c.code}</div>
                        <div style={{fontSize:9,color:"#64748b"}}>{c.note||"No note"} · {c.createdAt?new Date(c.createdAt).toLocaleDateString("en-LK"):""}</div>
                        {c.amount&&<div style={{fontSize:9,color:"#10b981"}}>Rs.{c.amount.toLocaleString()}</div>}
                      </div>
                      <div style={{display:"flex",gap:5,flexShrink:0}}>
                        <button onClick={()=>copyToClipboard(c.code)} style={{background:"#1e293b",color:"#94a3b8",border:"1px solid #334155",borderRadius:5,padding:"3px 7px",fontSize:9,cursor:"pointer"}}>📋</button>
                        <button onClick={()=>handleDeleteCode(c.code)} style={{background:"#7f1d1d",color:"#fca5a5",border:"none",borderRadius:5,padding:"3px 7px",fontSize:9,cursor:"pointer"}}>🗑</button>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {usedCodes.length>0&&(
              <div style={{...S.card,opacity:0.85,marginTop:8}}>
                <div style={{...S.cardTitle,color:"#64748b"}}>✅ Used Codes — {usedCodes.length}</div>
                <div style={{fontSize:9,color:"#334155",marginBottom:6}}>♻️ "Reissue" = allow same subscriber on new device</div>
                {usedCodes.map(c=>(
                  <div key={c.code} style={{padding:"8px 0",borderBottom:"1px solid #1e293b"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:12,fontWeight:"700",color:"#475569",fontFamily:"monospace",letterSpacing:1,textDecoration:"line-through"}}>{c.code}</div>
                        <div style={{fontSize:9,color:"#334155"}}>{c.note||""} · {c.usedBy?.name||c.usedBy?.mobile||"Unknown"} · {c.usedAt?new Date(c.usedAt).toLocaleDateString("en-LK"):""}</div>
                        {c.reissuedAt&&<div style={{fontSize:8,color:"#7c3aed"}}>♻️ Reissued: {new Date(c.reissuedAt).toLocaleDateString("en-LK")}</div>}
                      </div>
                      <button onClick={()=>handleReissueCode(c.code)}
                        style={{background:"#1a0040",color:"#a78bfa",border:"1px solid #7c3aed",borderRadius:5,padding:"3px 7px",fontSize:9,cursor:"pointer",fontWeight:"700"}}>
                        {T(lang,"reissue")}
                      </button>
                    </div>
                  </div>
            ...
            {allCodes.length===0&&<Empty icon="🔑" text="No codes yet. Generate one above."/>}
          </>
        )}

        {/* ═══ OTHER TABS ═══ */}
        {(view==="all"||view==="reg"||view==="pay")&&(
          ...
          displayed.length===0?<Empty icon="📭" text="No entries yet"/>:displayed.map(n=>(
            <div key={n.id} style={{...S.card,border:"1px solid "+(n.type==="PAYMENT_RECEIVED"?"#f59e0b":"#10b981"),marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                <span style={{fontSize:11,fontWeight:"700",color:n.type==="PAYMENT_RECEIVED"?"#fbbf24":"#34d399"}}>
              ...
              {n.type==="REGISTRATION"&&(
                <>
                  ...
                  {n.data?.tin&&<div style={{fontSize:10,color:"#64748b"}}>TIN: {n.data.tin}</div>}
                  <div style={{fontSize:10,color:"#64748b"}}>{n.data?.city} · {n.data?.employmentType==="freelancer"?"💻 Freelancer":"🏢 Salaried"}</div>
                  <button onClick={()=>{setPlanNote("Activation for "+n.data?.name);setNoteInput(n.data?.name||"");setView("codes");}}
                    style={{marginTop:8,padding:"6px 12px",background:"#1a0040",border:"1px solid #7c3aed",borderRadius:7,
              ...
              {n.type==="PAYMENT_RECEIVED"&&(
                <>
                  <div style={{fontSize:12,color:"#e2e8f0"}}><b>{n.data?.name}</b> · {n.data?.mobile}</div>
                  <div style={{fontSize:13,color:"#fbbf24",fontWeight:"700"}}>{fmtC(n.data?.amount||0)} · {n.data?.plan||""} · via {n.data?.activatedVia||"Manual"}</div>
                  {n.data?.code&&<div style={{fontSize:10,color:"#a78bfa"}}>Code: {n.data.code}</div>}
                  <div style={{fontSize:10,color:"#64748b"}}>Expires: {n.data?.expiryDate}</div>
                </>
              )}
            </div>
          ))
        ...
        {view==="pin"&&(
          <div style={S.card}>
            <div style={{...S.cardTitle,color:"#c4b5fd"}}>🔒 Change Admin PIN</div>
            {["current","new","confirm"].map(step=>(
              pinStep===step&&(
                <div key={step}>
                  <label style={{...S.lbl,color:"#a78bfa"}}>
                    {step==="current"?"Enter Current PIN":step==="new"?"New 4-digit PIN":"Confirm New PIN"}
                  </label>
                  <input type="password" maxLength={4} inputMode="numeric" placeholder="••••"
                    value={step==="current"?pinCurrent:step==="new"?pinNew:pinConfirm}
                    onChange={e=>{
                      const v=e.target.value.replace(/\D/g,"");
                      if(step==="current")setPinCurrent(v);
                      else if(step==="new")setPinNew(v);
                      else setPinConfirm(v);
                    }}
                    style={{...S.inp,letterSpacing:8,textAlign:"center",fontSize:18,background:"#0a0020",borderColor:"#4c1d95",color:"#e2e8f0"}}/>
                  <button onClick={()=>{
                    if(step==="confirm")handlePinChange();
                    else setPinStep(step==="current"?"new":"confirm");
                  }}
                    style={{...S.submitBtn,background:"linear-gradient(135deg,#7c3aed,#a78bfa)",width:"100%"}}>
                    {step==="confirm"?"✓ Change PIN":"Next →"}
                  </button>
                  {pinMsg.text&&<div style={{fontSize:12,fontWeight:"700",color:pinMsg.type==="error"?"#f87171":"#34d399",marginTop:8,textAlign:"center"}}>{pinMsg.text}</div>}
                </div>
              )
            ))}
            <div style={{fontSize:10,color:"#334155",marginTop:10,textAlign:"center"}}>Default PIN: 7749 · Change after first login</div>
          </div>
        )}
      </div>
    </div>
...
function Dashboard({cashBalance,totalBankBal,tax,data,profile,trialDaysLeft,onUpgrade,bankBalance,activeYear,switchYear}){
  ...
  return(
    <div style={S.page}>
      ...
      <div style={{...S.card,border:"1px solid #1e3a5f",background:"#030710",padding:"12px 14px",marginTop:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <img src={LOGO_B64} alt="" style={{width:28,height:28,borderRadius:"50%",border:"1.5px solid #b8960c",flexShrink:0}}/>
            <div>
              <div style={{fontSize:12,fontWeight:"800",color:"#f1f5f9"}}>FinTax LK</div>
              <div style={{fontSize:9,color:"#475569"}}>{FIRM_NAME}</div>
            </div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{
              ...
              fontSize:10, fontWeight:"800", color:"#f5d060",
            }}>v{APP_VERSION}</div>
            <div style={{fontSize:9,color:"#334155",marginTop:2}}>Latest · Feb 2026</div>
          </div>
        </div>
        <div style={{display:"flex",gap:6}}>
          <button
            onClick={()=>{
              if("serviceWorker" in navigator){
                navigator.serviceWorker.getRegistration("/sw.js").then(r=>{ if(r) r.update(); });
              }
              window.location.reload();
            }}
            style={{flex:1,padding:"8px",background:"#0a1525",border:"1px solid #1e3a5f",borderRadius:8,
              color:"#60a5fa",fontWeight:"700",fontSize:11,cursor:"pointer"}}>
            🔄 Check for Updates
          </button>
          <a href="https://fintaxlk.pages.dev" target="_blank"
            style={{flex:1,padding:"8px",background:"#0a2a1a",border:"1px solid #064e3b",borderRadius:8,
              color:"#34d399",fontWeight:"700",fontSize:11,textDecoration:"none",textAlign:"center"}}>
            🌐 App Website
          </a>
        </div>
        <ContactLinks small/>
  ...
  {showForm ? "✕" : (data.incomeEntries.length===0 ? "✨ Add Income" : "+ Add")}
      ...
      {showForm&&(
        <FormCard title="Add Income Entry">
          <FS label="Income Type" value={form.category} options={incomeCats} onChange={v=>setForm(f=>({...f,category:v,invoiceNo:"",clientName:""}))}/>
          ...
          {form.category==="Invoice/Client Fees"&&(
            <>
              <FI label="Invoice No." value={form.invoiceNo||""} onChange={v=>setForm(f=>({...f,invoiceNo:v}))} placeholder="INV-001"/>
              <FI label="Client / Company Name" value={form.clientName||""} onChange={v=>setForm(f=>({...f,clientName:v}))} placeholder="e.g. XYZ Technologies Pvt Ltd"/>
              <div style={{background:"#0a1a14",border:"1px solid #10b981",borderRadius:8,padding:"8px 10px",marginBottom:10,fontSize:10,color:"#6ee7b7"}}>
                🧾 Invoice income is included in your taxable income. Applicable APIT or WHT withheld by client is creditable.
              </div>
            </>
          )}
          ...
          <FI label="Description / Source" value={form.description}
            onChange={v=>setForm(f=>({...f,description:v}))}
            placeholder={form.category==="Invoice/Client Fees"?"e.g. Web design services — Jan 2025":"e.g. ABC Company monthly salary"}/>
          {(form.category!=="Freelance/Contract"&&!isFreelancer)||form.category==="Invoice/Client Fees"?
            <FI label="APIT / WHT Deducted by Payer (Rs.)" type="number" value={form.apitAmount} onChange={v=>setForm(f=>({...f,apitAmount:v}))} placeholder="If client deducted tax, enter here"/>:
  ...
  const downloadReport = ()=>{
    ...
    const txnRows = rows.map((t,i)=>`
      <tr>
        ...
        <td class="${t.runBal<0?'neg':'bal'}">Rs.${t.runBal.toLocaleString("en-LK",{minimumFractionDigits:2})}</td>
      </tr>`).join("");
...
.bal{color:#1e40af;font-weight:700;text-align:right}.neg{color:#dc2626;font-weight:700;text-align:right}
.section{font-size:14px;font-weight:800;color:#1a1a2e;margin:20px 0 10px;padding-left:6px;border-left:4px solid #b8960c}
.total-row td{background:#1a1a2e!important;color:#f5d060;font-weight:800;font-size:13px}
.total-row .g{color:#34d399}.total-row .r{color:#f87171}.total-row .bal{color:#60a5fa}.total-row .neg{color:#f87171}
.disclaimer{background:#fef3c7;border:1px solid #fbbf24;border-radius:8px;padding:12px;font-size:11px;color:#78350f;margin-bottom:20px}
.footer{margin-top:30px;border-top:2px solid #b8960c;padding-top:16px;display:flex;justify-content:space-between;font-size:10px;color:#94a3b8}
@media print{body{padding:0}button{display:none}}
        ...
        {filtered.length===0?(
          ...
          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 12px",borderBottom:"1px solid #0a1f2e"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2,flexShrink:0,marginLeft:8}}>
              <div style={{fontSize:13,fontWeight:"700",color:t.type==="income"?"#34d399":"#f87171"}}>
                {t.type==="income"?"+":"-"}{fmt(t.amount)}
              </div>
              {t.medium==="cash"&&(
                <div style={{fontSize:9,color:t.runBal<0?"#f87171":"#475569",fontFamily:"monospace",
                  fontWeight:t.runBal<0?"800":"400"}}>
                  Bal: {t.runBal<0?"-":""}{fmtC(Math.abs(t.runBal))}
                </div>
              )}
            </div>
          </div>
...
// ─── APIT CALCULATOR PAGE ─────────────────────────────────────────────────────
function APitCalculatorPage(){
  const [tabYear,setTabYear]=useState("2025/2026");
  const [empType,setEmpType]=useState("primary"); // primary | secondary | lumpsum
  const [salary,setSalary]=useState("");
  const [residence,setResidence]=useState(false);
  const [vehicle,setVehicle]=useState(false);
  const [result,setResult]=useState(null);

  // Official APIT Table 01 monthly slabs (Tax Table No. 01)
  const APIT_TABLES={
    "2025/2026":{
      threshold:150_000,
      slabs:[
        {max:150_000,  f:()=>0,                         pct:"0%",   desc:"≤ Rs.150,000 — Tax Free"},
        {max:233_333,  f:(p)=>Math.max(0,p*0.06-9000),  pct:"6%",   desc:"Rs.150,001–233,333"},
        {max:275_000,  f:(p)=>Math.max(0,p*0.18-37000), pct:"18%",  desc:"Rs.233,334–275,000"},
        {max:316_667,  f:(p)=>Math.max(0,p*0.24-53500), pct:"24%",  desc:"Rs.275,001–316,667"},
        {max:358_333,  f:(p)=>Math.max(0,p*0.30-72500), pct:"30%",  desc:"Rs.316,668–358,333"},
        {max:Infinity, f:(p)=>Math.max(0,p*0.36-94000), pct:"36%",  desc:"Above Rs.358,333"},
      ],
      relief:150_000,
      label:"Y/A 2025/2026 — IRA Amendment No.2/2025 (from 01 Apr 2025)",
      note:"New relief Rs.1,800,000/yr. WHT on interest now 10%.",
    },
    "2024/2025":{
      threshold:100_000,
      slabs:[
        {max:100_000,  f:()=>0,                           pct:"0%",   desc:"≤ Rs.100,000 — Tax Free"},
        {max:141_667,  f:(p)=>Math.max(0,p*0.06-6000),    pct:"6%",   desc:"Rs.100,001–141,667"},
        {max:175_000,  f:(p)=>Math.max(0,p*0.18-23000),   pct:"18%",  desc:"Rs.141,668–175,000"},
        {max:208_333,  f:(p)=>Math.max(0,p*0.24-33500),   pct:"24%",  desc:"Rs.175,001–208,333"},
        {max:241_667,  f:(p)=>Math.max(0,p*0.30-50000),   pct:"30%",  desc:"Rs.208,334–241,667"},
        {max:Infinity, f:(p)=>Math.max(0,p*0.36-64500),   pct:"36%",  desc:"Above Rs.241,667"},
      ],
      relief:100_000,
      label:"Y/A 2024/2025 (01 Apr 2024 – 31 Mar 2025)",
      note:"Relief Rs.1,200,000/yr. WHT on interest 5%.",
    },
    "2023/2024":{
      threshold:100_000,
      slabs:[
      ...
      relief:100_000,
      label:"Y/A 2023/2024 (01 Apr 2023 – 31 Mar 2024)",
      note:"Same as 2024/25. Relief Rs.1,200,000/yr.",
    },
  };

  const calcAPIT=()=>{
    const base=parseFloat(salary)||0;
    const resBenefit=residence?base*0.125:0;
    const vehBenefit=vehicle?50_000:0;
    const totalMonthly=base+resBenefit+vehBenefit;

    const tbl=APIT_TABLES[tabYear];
    if(!tbl){setResult(null);return;}

    const slab=tbl.slabs.find(s=>totalMonthly<=s.max);
    const tax=slab?slab.f(totalMonthly):0;
    const netTakeHome=base-tax;
    const effectiveRate=totalMonthly>0?(tax/totalMonthly*100):0;

    setResult({
      base,resBenefit,vehBenefit,totalMonthly,tax,netTakeHome,effectiveRate,
      slab,annualTax:tax*12,annualGross:totalMonthly*12,
      tbl,
    });
  };

  const tbl=APIT_TABLES[tabYear]||APIT_TABLES["2025/2026"];

  return(
    <FinPage icon="🧮" title="APIT Calculator" subtitle="Advance Personal Income Tax — Tax Table No.01 (Primary Employment)">

      {/* Year Tabs */}
      <div style={{display:"flex",gap:4,marginBottom:12,flexWrap:"wrap"}}>
        {Object.keys(APIT_TABLES).map(yr=>(
          <button key={yr} onClick={()=>{setTabYear(yr);setResult(null);}}
            style={{...S.tBtn,flexShrink:0,fontSize:10,
              ...(tabYear===yr?{background:F.gradGold,borderColor:F.accentGold,color:F.accentGold}:{})}}>
            {yr}
          </button>
        ))}
      </div>

      <div style={{background:"#0a1525",border:"1px solid #1e3a5f",borderRadius:8,padding:"8px 10px",marginBottom:12}}>
        <div style={{fontSize:10,color:"#60a5fa",fontWeight:"700"}}>{tbl.label}</div>
        <div style={{fontSize:9,color:"#475569",marginTop:2}}>{tbl.note}</div>
        <div style={{fontSize:9,color:"#334155",marginTop:2}}>
          Source: IRD Sri Lanka — ird.gov.lk | APIT Tax Table No. 01
        </div>
      </div>

      {/* Tax Table Reference */}
      <div style={{...S.card,marginBottom:12}}>
        <div style={{...S.cardTitle,fontSize:11}}>📋 Monthly Tax Table (Primary Employment)</div>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}>
            <thead>
              <tr style={{borderBottom:"1px solid #1e293b"}}>
                <th style={{textAlign:"left",padding:"4px 6px",color:"#64748b"}}>Monthly Income</th>
                <th style={{textAlign:"center",padding:"4px 4px",color:"#64748b"}}>Rate</th>
                <th style={{textAlign:"right",padding:"4px 6px",color:"#64748b"}}>Formula</th>
              </tr>
            </thead>
            <tbody>
              {tbl.slabs.map((s,i)=>(
                <tr key={i} style={{borderBottom:"1px solid #0d1829",background:i%2===0?"#070d18":"transparent"}}>
                  <td style={{padding:"4px 6px",color:"#cbd5e1",fontSize:9}}>{s.desc}</td>
                  <td style={{textAlign:"center",padding:"4px 4px",fontWeight:"700",
                    color:s.pct==="0%"?"#10b981":s.pct==="6%"?"#fbbf24":s.pct==="18%"?"#f59e0b":s.pct==="24%"?"#fb923c":s.pct==="30%"?"#f87171":"#ef4444"}}>
                    {s.pct}
                  </td>
                  <td style={{textAlign:"right",padding:"4px 6px",color:"#64748b",fontFamily:"monospace",fontSize:8}}>
                    {s.pct==="0%"?"T = 0":
                     tabYear==="2025/2026"?(
                       s.pct==="6%"?"T = P×6%−9,000":s.pct==="18%"?"T = P×18%−37,000":
                       s.pct==="24%"?"T = P×24%−53,500":s.pct==="30%"?"T = P×30%−72,500":"T = P×36%−94,000"
                     ):(
                       s.pct==="6%"?"T = P×6%−6,000":s.pct==="18%"?"T = P×18%−23,000":
                       s.pct==="24%"?"T = P×24%−33,500":s.pct==="30%"?"T = P×30%−50,000":"T = P×36%−64,500"
                     )
                    }
                  </td>
                </tr>
        ...
        <div style={{fontSize:9,color:"#334155",marginTop:6}}>
          P = Monthly Regular Profits from Primary Employment · T = Monthly APIT Deductible
        </div>
      </div>

      {/* Calculator Inputs */}
      <FormCard title="Calculate Your Monthly APIT">
        <FI label={`Base Monthly Salary / Profits (Rs.)`} type="number" value={salary}
          onChange={v=>{setSalary(v);setResult(null);}} placeholder="e.g. 250000"/>

        <div style={{marginBottom:12}}>
          <div style={{fontSize:11,fontWeight:"700",color:F.textSecondary,marginBottom:8}}>Non-Cash Benefits (IRD Quantification)</div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",
              background:residence?F.gradGold:"#0a0f1a",border:`1px solid ${residence?F.accentGold:F.border}`,
              borderRadius:8,padding:"10px 12px"}}>
              <input type="checkbox" checked={residence} onChange={e=>{setResidence(e.target.checked);setResult(null);}}
                style={{width:16,height:16,accentColor:F.accentGold}}/>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:"700",color:residence?F.accentGold:F.textSecondary}}>
                  🏠 Company-Provided Residence
                </div>
                <div style={{fontSize:9,color:F.textMuted}}>
                  Benefit = 12.5% of monthly salary
                  {salary&&residence?` = Rs.${(parseFloat(salary)*0.125).toLocaleString("en-LK",{maximumFractionDigits:0})}/month`:""}
                </div>
              </div>
            </label>

            <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",
              background:vehicle?F.gradGold:"#0a0f1a",border:`1px solid ${vehicle?F.accentGold:F.border}`,
              borderRadius:8,padding:"10px 12px"}}>
              <input type="checkbox" checked={vehicle} onChange={e=>{setVehicle(e.target.checked);setResult(null);}}
                style={{width:16,height:16,accentColor:F.accentGold}}/>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:"700",color:vehicle?F.accentGold:F.textSecondary}}>
                  🚗 Company Vehicle (Private Use)
                </div>
                <div style={{fontSize:9,color:F.textMuted}}>Fixed benefit Rs.50,000/month (Rs.20k vehicle + Rs.10k driver + Rs.20k fuel)</div>
              </div>
            </label>
          </div>
        </div>
        ...
        {salary&&(parseFloat(salary)||0)>0&&(
          <div style={{background:"#0a1a14",border:"1px solid #10b981",borderRadius:8,padding:"8px 10px",marginBottom:10,fontSize:10,color:"#6ee7b7"}}>
            Total Monthly Profits (P) = <strong>Rs.{((parseFloat(salary)||0)+(residence?(parseFloat(salary)||0)*0.125:0)+(vehicle?50000:0)).toLocaleString("en-LK")}</strong>
          </div>
        )}

        <button style={{...S.submitBtn,background:"linear-gradient(135deg,#b8960c,#f5d060)",color:"#111"}} onClick={calcAPIT}>
          🧮 Calculate APIT
        </button>
      </FormCard>
      ...
      {result&&(
        <div style={S.card}>
          <div style={{...S.cardTitle,color:F.accentGold}}>📊 APIT Calculation Result</div>

          <div style={{background:"linear-gradient(135deg,#0d0d00,#1a1500)",border:`2px solid ${F.accentGold}`,
            borderRadius:12,padding:14,marginBottom:12,textAlign:"center"}}>
            <div style={{fontSize:11,color:F.textMuted,marginBottom:4}}>Monthly APIT Deduction</div>
            <div style={{fontSize:32,fontWeight:"900",color:"#f87171"}}>Rs.{result.tax.toLocaleString("en-LK",{maximumFractionDigits:0})}</div>
            <div style={{fontSize:11,color:F.textMuted,marginTop:4}}>Net Take-Home: <strong style={{color:"#34d399"}}>Rs.{result.netTakeHome.toLocaleString("en-LK",{maximumFractionDigits:0})}</strong></div>
            <div style={{fontSize:10,color:"#64748b",marginTop:2}}>Effective Rate: {result.effectiveRate.toFixed(1)}%</div>
          </div>

          {result.resBenefit>0||result.vehBenefit>0?(<div style={{marginBottom:10}}>
            <TRow l="Base Salary" v={fmtC(result.base)} c={F.textSecondary}/>
            {result.resBenefit>0&&<TRow l="+ Residence Benefit (12.5%)" v={fmtC(result.resBenefit)} c="#f59e0b"/>}
            {result.vehBenefit>0&&<TRow l="+ Vehicle Benefit (fixed)" v={fmtC(result.vehBenefit)} c="#f59e0b"/>}
            <TRow l="= Total Monthly Profits (P)" v={fmtC(result.totalMonthly)} c={F.accentGold} bold/>
          </div>):null}

          <div style={S.divider}/>
          <TRow l="Applicable Tax Slab" v={result.slab?.pct||"—"} c={F.accentPrimary}/>
          <TRow l="Monthly APIT" v={fmtC(result.tax)} c="#f87171" bold/>
          <TRow l="Annual APIT (×12)" v={fmtC(result.annualTax)} c="#f87171"/>
          <div style={S.divider}/>
          <TRow l="Monthly Net Income" v={fmtC(result.netTakeHome)} c="#34d399" bold/>
          <TRow l="Annual Net Income" v={fmtC(result.netTakeHome*12)} c="#34d399"/>

          <div style={{background:"#0a1525",border:"1px solid #1e3a5f",borderRadius:8,padding:"8px 10px",marginTop:10}}>
            <div style={{fontSize:10,color:"#60a5fa",fontWeight:"700",marginBottom:4}}>ℹ️ Important Notes</div>
            <div style={{fontSize:9,color:"#475569",lineHeight:1.7}}>
              • This uses Tax Table No. 01 (primary employment, regular income only)<br/>
              • Bonuses/lump-sum use Tax Table No. 02 (different formula)<br/>
              • Employer must remit APIT to IRD by 15th of following month<br/>
              • Secondary employment uses Tax Table No. 07 at higher rates<br/>
              • Verify with a GDP Consultants tax advisor for complex situations
            </div>
          </div>
        </div>
      )}

      <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${encodeURIComponent("Hi GDP Consultants, I'd like to verify my APIT calculation with a tax advisor.")}`}
        target="_blank"
        style={{display:"block",marginTop:8,background:"linear-gradient(135deg,#b8960c,#f5d060)",borderRadius:10,padding:"10px",color:"#111",fontWeight:"800",fontSize:12,textDecoration:"none",textAlign:"center"}}>
        📞 Get Expert APIT Advice — WhatsApp Us
      </a>
    </FinPage>
  );
}
...
function ToolsPage({data, update, sub, lang, tax, profile}) {
  ...
  if(subTool==="apit")     return <><APitCalculatorPage/><BackBtn onClick={()=>setSubTool(null)}/></>;
  ...
  const TOOLS = [
    ...
    {
      id:"apit", icon:"🧮", title:"APIT Calculator",
      desc:"Official IRD Tax Table No.01 — calculate monthly APIT deduction for any year from 2022/23 to 2025/26",
      badge:"NEW", badgeColor:"#059669", tier:"Tier 1",
      accent:"#34d399", bg:"linear-gradient(135deg,#01160a,#022010)",
    },
    {
      id:"apit", icon:"🧮", title:"APIT Calculator",
      desc:"Official IRD Tax Table No.01 — compute monthly APIT for 2023/24, 2024/25, 2025/26",
      badge:"NEW", badgeColor:"#059669", tier:"Tier 1",
      accent:"#34d399", bg:"linear-gradient(135deg,#010d08,#021a10)",
    },
    {
...
function ServicesPage({profile,lang}){
  ...
  // Current services with pricing
  const CURRENT_SERVICES=[
    {icon:"⚠️",urgent:true,color:"#ef4444",border:"#7f1d1d",bg:"linear-gradient(135deg,#1a0000,#2d0000)",
     title:"IRD Notice Response",price:"Rs.5,000–10,000",desc:"Received an IRD notice? We respond on your behalf within 24 hours. Experienced with all IRD communication types.",
     waMsg:"Hi GDP Consultants, I received an IRD notice and need URGENT help responding. Please call me."},
    {icon:"📝",color:"#3b82f6",border:"#1e3a5f",bg:"linear-gradient(135deg,#050e1a,#0a1a30)",
     title:"VAT Registration",price:"Rs.8,000",desc:"Get your business VAT-registered with IRD. We handle all paperwork and submission.",
     waMsg:"Hi GDP Consultants, I'd like to register my business for VAT."},
    {icon:"🧮",color:"#10b981",border:"#064e3b",bg:"linear-gradient(135deg,#010d08,#021a10)",
     title:"Tax Return Filing (AITR)",price:"Rs.5,000",desc:"We prepare and file your Annual Income Tax Return with IRD. Accuracy guaranteed — FREE for Pro/Premium subscribers.",
     waMsg:"Hi GDP Consultants, I need help filing my Annual Income Tax Return (AITR)."},
    {icon:"🏢",color:"#a78bfa",border:"#4c1d95",bg:"linear-gradient(135deg,#0a0520,#15073a)",
     title:"Company Registration",price:"Rs.15,000",desc:"Register a private limited company end-to-end including ROC filing, Articles, and Secretary appointment.",
     waMsg:"Hi GDP Consultants, I'd like to register a private limited company."},
    {icon:"🔍",color:"#f59e0b",border:"#92400e",bg:"linear-gradient(135deg,#1a0e00,#2a1500)",
     title:"Statutory Audit Certificate",price:"From Rs.25,000",desc:"Statutory audit certificate for your business. Required by banks, government tenders, and regulatory filings.",
     waMsg:"Hi GDP Consultants, I need a statutory audit certificate for my business."},
    {icon:"✏️",color:"#10b981",border:"#064e3b",bg:"linear-gradient(135deg,#010d08,#021a10)",
     title:"Amended Tax Return",price:"Rs.5,000",desc:"Fix errors in a previously filed tax return. We liaise with IRD for correction and resubmission.",
     waMsg:"Hi GDP Consultants, I need to file an amended tax return."},
    {icon:"💰",color:"#06b6d4",border:"#0e7490",bg:"linear-gradient(135deg,#01101a,#011a28)",
     title:"Payroll / APIT Management",price:"Rs.2,000/emp/mo",desc:"Monthly APIT payroll for you and your employees. Includes deduction schedules, PAYE returns, and EPF/ETF remittances.",
     waMsg:"Hi GDP Consultants, I need help with APIT payroll management for my business."},
    {icon:"📋",color:"#f59e0b",border:"#92400e",bg:"linear-gradient(135deg,#1a0e00,#2a1500)",
     title:"Management Accounts",price:"Rs.8,000/qtr",desc:"Quarterly management accounts: P&L, Balance Sheet, and cash flow statement for your business decision-making.",
     waMsg:"Hi GDP Consultants, I'd like quarterly management accounts prepared for my business."},
  ];

  const ADDITIONAL_SERVICES=[
    {icon:"🌐",color:"#8b5cf6",border:"#5b21b6",bg:"linear-gradient(135deg,#0a0520,#150a3a)",
     title:"Transfer Pricing Documentation",price:"From Rs.35,000",desc:"Mandatory TP documentation for related-party transactions under IRD Ruling 2024/01. Protects against reassessment.",
     waMsg:"Hi GDP Consultants, I need transfer pricing documentation for related-party transactions."},
    {icon:"🏠",color:"#f59e0b",border:"#92400e",bg:"linear-gradient(135deg,#1a0e00,#2a1500)",
     title:"Rental Income Tax Advisory",price:"Rs.3,500",desc:"Optimise tax on rental income: allowable deductions, assessable income calculation, and withholding tax guidance.",
     waMsg:"Hi GDP Consultants, I need tax advice on my rental income."},
    {icon:"💹",color:"#10b981",border:"#064e3b",bg:"linear-gradient(135deg,#010d08,#021a10)",
     title:"Investment & WHT Refund",price:"Rs.4,000",desc:"Claim back overpaid withholding tax on FD interest, dividends. We compute the refund and file on your behalf.",
     waMsg:"Hi GDP Consultants, I'd like to claim a WHT refund on my investment income."},
    {icon:"🌍",color:"#3b82f6",border:"#1e3a5f",bg:"linear-gradient(135deg,#050e1a,#0a1a30)",
     title:"Export / Foreign Income Tax",price:"Rs.6,000",desc:"15% flat tax on service exports & foreign-sourced income (Act No.2/2025). We compute, plan and file correctly.",
     waMsg:"Hi GDP Consultants, I need help with tax on my export / foreign income (15% flat rate)."},
    {icon:"📊",color:"#a78bfa",border:"#4c1d95",bg:"linear-gradient(135deg,#0a0520,#15073a)",
     title:"Due Diligence Review",price:"From Rs.40,000",desc:"Financial due diligence for acquisitions, investments or partnerships. Review of books, tax compliance and liabilities.",
     waMsg:"Hi GDP Consultants, I need a due diligence review for a business acquisition."},
    {icon:"📑",color:"#06b6d4",border:"#0e7490",bg:"linear-gradient(135deg,#01101a,#011a28)",
     title:"Business Valuation Report",price:"From Rs.30,000",desc:"Professional valuation using DCF, market multiples and asset-based approaches. Required for bank loans and M&A.",
     waMsg:"Hi GDP Consultants, I need a business valuation report."},
    {icon:"🔒",color:"#ef4444",border:"#7f1d1d",bg:"linear-gradient(135deg,#1a0000,#2d0000)",
     title:"Internal Audit & Controls",price:"From Rs.20,000",desc:"Review of internal controls and risk management. Identify gaps before the statutory auditor does.",
     waMsg:"Hi GDP Consultants, I'm interested in an internal audit review for my business."},
    {icon:"🤝",color:"#b8960c",border:"#78650a",bg:"linear-gradient(135deg,#0d0d00,#1a1500)",
     title:"Company Secretarial Services",price:"Rs.3,000/mo",desc:"Annual returns, board resolutions, AGM filing, share transfers and ROC compliance maintained monthly.",
     waMsg:"Hi GDP Consultants, I need ongoing company secretarial services."},
    {icon:"🧑‍💻",color:"#8b5cf6",border:"#5b21b6",bg:"linear-gradient(135deg,#0a0520,#150a3a)",
     title:"Freelancer Tax Package",price:"Rs.10,000/yr",desc:"Complete freelancer/IT contractor tax solution: quarterly advance tax, AITR filing, foreign income reporting, and 1hr advisory.",
     waMsg:"Hi GDP Consultants, I'm a freelancer and need help with my tax planning and filing."},
    {icon:"🏭",color:"#f59e0b",border:"#92400e",bg:"linear-gradient(135deg,#1a0e00,#2a1500)",
     title:"VAT & NBT Compliance",price:"Rs.5,000/mo",desc:"VAT and Nation Building Tax compliance: monthly returns, filing and reconciliation with IRD requirements.",
     waMsg:"Hi GDP Consultants, I need help with VAT and NBT compliance obligations."},
  ];

  const SvcCard=({svc})=>(
    <div style={{background:svc.bg,border:`1px solid ${svc.border}`,borderRadius:12,padding:"12px 14px",marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
        <div style={{display:"flex",gap:8,alignItems:"flex-start",flex:1}}>
          <span style={{fontSize:20,flexShrink:0,marginTop:2}}>{svc.icon}</span>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:"800",color:svc.color}}>{svc.title}</div>
            <div style={{fontSize:10,color:"#64748b",marginTop:2,lineHeight:1.5}}>{svc.desc}</div>
          </div>
        </div>
        <div style={{fontSize:12,fontWeight:"800",color:"#f5d060",flexShrink:0,marginLeft:8,textAlign:"right"}}>{svc.price}</div>
      </div>
      <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${encodeURIComponent(svc.waMsg+" My name is "+(profile?.name||"")+", mobile: "+(profile?.mobile||""))}`}
        target="_blank"
        style={{display:"block",background:"#064e3b",border:"1px solid #10b981",borderRadius:8,padding:"8px",
          color:"#6ee7b7",fontWeight:"700",fontSize:12,textDecoration:"none",textAlign:"center"}}>
        💬 Book via WhatsApp
      </a>
    </div>
  );
  ...
  return(
    <div style={S.page}>
      <div style={S.pageTitle}>💼 Services & Pricing</div>
      <div style={{fontSize:11,color:"#64748b",marginBottom:16}}>
        Professional accounting, tax & audit services · GDP Consultants · WhatsApp for instant booking
      </div>

      {/* IRD Urgent Banner */}
      <div style={{background:"linear-gradient(135deg,#7f1d1d,#991b1b)",border:"2px solid #ef4444",
        borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",gap:12,alignItems:"center"}}>
        <span style={{fontSize:28,flexShrink:0}}>⚠️</span>
        <div style={{flex:1}}>
          <div style={{fontSize:14,fontWeight:"800",color:"#fca5a5"}}>Received an IRD Notice?</div>
          <div style={{fontSize:11,color:"#fca5a5",opacity:0.8,marginBottom:8}}>
            Act quickly — delays increase penalties. We respond on your behalf within 24 hours.
          </div>
          <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${encodeURIComponent("Hi GDP Consultants, I received an IRD notice and need URGENT help responding. Please call me. Name: "+(profile?.name||""))}`}
            target="_blank"
            style={{display:"inline-block",background:"#ef4444",color:"#fff",border:"none",
              borderRadius:8,padding:"8px 16px",fontWeight:"800",fontSize:13,textDecoration:"none"}}>
            🚨 Get Urgent Help Now
          </a>
        </div>
      </div>
      ...
      {/* App Tools Strip */}
      <div style={{...S.card,border:"1px solid #1e3a5f",background:"linear-gradient(135deg,#050e1a,#0a1a30)",marginBottom:14}}>
        <div style={{fontSize:12,fontWeight:"800",color:"#60a5fa",marginBottom:10}}>🔧 Tools Included in Your FinTax LK Subscription</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {[
            "📊 IIT Tax Calculator (all years)",
            "🧮 APIT Calculator (2023–2026)",
            "💵 Cash Book & Bank Tracker",
            "📥 Income Sources (Invoice/Salary)",
            "🏦 EPF Retirement Planner",
            "🏠 Loan & Mortgage Calculator",
            "📋 VAT Threshold Monitor",
            "📈 Investment & WHT Tracker",
            "⏰ Advance Tax Planner",
            "🏘️ Rental Property Manager",
            "💎 Net Worth Dashboard",
            "📑 Personal Finance Report",
            "🤖 AI Tax Assistant",
          ].map(t=>(
            <div key={t} style={{fontSize:9,color:"#93c5fd",background:"#0a1525",border:"1px solid #1e3a5f",
        ...
        <div style={{fontSize:10,color:"#334155",marginTop:8}}>
          All tools included with any subscription · App available 24/7
        </div>
      </div>

      {/* Current Professional Services */}
      <div style={{fontSize:13,fontWeight:"800",color:"#f1f5f9",marginBottom:10}}>
        📋 Professional Services — Current Pricing
      </div>
      {CURRENT_SERVICES.map((svc,i)=><SvcCard key={i} svc={svc}/>)}

      {/* Additional / New Services */}
      <div style={{marginTop:6,marginBottom:10}}>
        <button onClick={()=>setShowAll(v=>!v)}
          style={{width:"100%",padding:"11px",background:showAll?"#0a1525":"linear-gradient(135deg,#1a1200,#2a1c00)",
            border:"1px solid "+(showAll?"#1e3a5f":"#b8960c"),borderRadius:10,
            color:showAll?"#64748b":"#f5d060",fontWeight:"800",fontSize:13,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          {showAll?"▲ Hide Additional Services":"▼ Show More Services (10 more)"}
        </button>
      </div>
      {showAll&&(
        <>
          <div style={{fontSize:13,fontWeight:"800",color:"#a78bfa",marginBottom:10}}>
            🚀 Additional Services — Accounting / Tax / Audit
          </div>
          {ADDITIONAL_SERVICES.map((svc,i)=><SvcCard key={i} svc={svc}/>)}
        </>
      )}
      ...
      <div style={{...S.card,border:"1px solid #b8960c",background:"linear-gradient(160deg,#0d0d00,#1a1500)",marginTop:6}}>
        ...
        <div style={{fontSize:12,color:"#94a3b8",lineHeight:1.7,marginBottom:10}}>
          For every friend who subscribes, you get <span style={{color:"#34d399",fontWeight:"700"}}>1 month FREE</span> added to your account.
        </div>
        {["1. Share the app with a friend or colleague","2. They subscribe and mention your name","3. WhatsApp us — we add your free month instantly"].map((s,i)=>(
          <div key={i} style={{fontSize:12,color:"#f5d060",padding:"3px 0"}}>{s}</div>
        ))}
        <a href={`https://wa.me/94${APP_WHATSAPP.replace(/^0/,"")}?text=${referralMsg}`} target="_blank"
          style={{display:"block",marginTop:12,background:"linear-gradient(135deg,#b8960c,#f5d060)",
            borderRadius:8,padding:"10px",color:"#111",fontWeight:"800",fontSize:13,textDecoration:"none",textAlign:"center"}}>
          🎁 Claim My Referral Reward
        </a>
      </div>

      {/* Bank details reminder */}
      <div style={{...S.card,border:"1px solid #1e3a5f",background:"linear-gradient(135deg,#050e1a,#0a1a30)",marginTop:8}}>
        <div style={{fontSize:11,fontWeight:"800",color:"#60a5fa",marginBottom:8}}>🏦 Payment Details</div>
        {BANK_ACCOUNTS.map((acc,i)=>(
          <div key={i}>
            <div style={{fontSize:11,color:"#93c5fd",fontWeight:"700"}}>{acc.bank}</div>
            <div style={{fontSize:10,color:"#64748b",lineHeight:1.8}}>
              <span style={{color:"#e2e8f0"}}>A/C Name: </span>{acc.acName}<br/>
              <span style={{color:"#e2e8f0"}}>A/C No: </span><span style={{fontFamily:"monospace",color:"#f5d060"}}>{acc.acNo}</span><br/>
              <span style={{color:"#e2e8f0"}}>SWIFT: </span>{acc.swift}<br/>
              <span style={{color:"#e2e8f0"}}>Branch: </span>{acc.branch}
            </div>
          </div>
        ))}
        <div style={{fontSize:10,color:"#334155",marginTop:8}}>
          After deposit: WhatsApp payment slip to {APP_WHATSAPP} · Activation within 2 hours
        </div>
      </div>
...
const catIcon=c=>({Salary:"💼",Employment:"💼","Freelance/Contract":"💻","Invoice/Client Fees":"🧾","Business Income":"🏪",Rental:"🏠",Investment:"📈","Export/Foreign":"🌐","Other Income":"💰"}[c]||"💰");
