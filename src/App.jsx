...
// ─── Tax Constants — Y/A 2025/2026 (Act No. 2 of 2025) ───────────────────────
...
// Salaried IIT Slabs (on taxable income after personal relief)
const IIT_SLABS = [
...
function computeTax(profile, incomeEntries, expenseEntries, taxSettings) {
  ...
  const relief       = isNRNC ? 0 : PERSONAL_RELIEF;
  const qpRelief     = isNRNC ? 0 : Math.min(taxSettings?.qualifyingPayments||0, QP_CAP);
...
// ── PWA: inject manifest + register service worker ───────────────────────────
  ...
  const [data,setData] = useState({
    transactions:[],      // unified cash+bank book
    incomeEntries:[],
    expenseEntries:[],    // freelancer business expenses
    bankAccounts:[],
  ...
  useEffect(()=>{
    (async()=>{
      const[stored,savedSub,savedProfile,savedLang]=await Promise.all([load("fintax_v4"),load("fintax_sub_v4"),load("fintax_profile_v4"),load("fintax_lang")]);
  ...
  const update=useCallback((patch)=>{
    setData(prev=>{ const next={...prev,...patch}; save("fintax_v4",next); return next; });
  },[]);
  ...
  // Secret admin: tap logo 7× in 3s
  const handleLogoTap=()=>{
    setLogoTaps(t=>t+1);
    clearTimeout(logoTapTimer.current);
    logoTapTimer.current=setTimeout(()=>setLogoTaps(0),3000);
    if(logoTaps>=6) setAdminMode(true);
  };
  ...
  const tax          = computeTax(profile, data.incomeEntries, data.expenseEntries, data.taxSettings);

  const props={data,update,profile,setProfile:saveProfile,tax,cashBalance,totalBankBal,bankBalance,trialDaysLeft,onUpgrade:activatePaid,onActivateCode:activateWithCode,sub,incomeCats,handleLogoTap,lang,setLang,t};

  return(
    <div style={S.phone}>
      <StatusBar trialDaysLeft={trialDaysLeft} profile={profile} onEditProfile={()=>setScreen("onboard")} onLogoTap={handleLogoTap} lang={lang} setLang={setLang}/>
      <div style={S.screen}>
...
function AdminPanel({onExit,lang,setLang}){
  ...
  const [view,setView]=useState("codes"); // codes | all | reg | pay
    ...
    <div style={S.phone}>
      ...
      <div style={{padding:"14px",overflowY:"auto",flex:1,paddingBottom:24}}>
        ...
        <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}}>
          {[["codes","🔑 Codes"],["all","📋 All"],["reg","👤 Users"],["pay","💳 Payments"]].map(([v,l])=>(
            <button key={v} style={{...S.tBtn,flexShrink:0,...(view===v?{background:"#1a0050",borderColor:"#7c3aed",color:"#c4b5fd"}:{})}} onClick={()=>setView(v)}>{l}</button>
          ))}
...
// ─── STATUS BAR ────────────────────────────────────────────────────────────────
function StatusBar({trialDaysLeft,profile,onEditProfile,onLogoTap,lang,setLang}){
  return(
    <div style={S.statusBar}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:5}}>
        ...
        {profile&&<span style={{fontSize:9,background:"#1e293b",color:"#64748b",padding:"2px 6px",borderRadius:4}}>{profile.employmentType==="freelancer"?"💻 FL":"🏢 SAL"}</span>}
...
function NavBar({tab,setTab,lang}){
  const tabs=[
    ...
    {id:"book",     icon:"📒",key:"nav_book"},
    {id:"income",   icon:"📥",key:"nav_income"},
    {id:"tax",      icon:"📋",key:"nav_tax"},
  ...
  return(
    <nav style={S.nav}>
      {tabs.map(tb=>(
        <button key={tb.id} style={{...S.navBtn,...(tab===tb.id?{background:"#0a1228"}:{})}} onClick={()=>setTab(tb.id)}>
          <span style={{fontSize:18}}>{tb.icon}</span>
          <span style={{fontSize:8,marginTop:1,color:tab===tb.id?"#b8960c":"#475569"}}>{T(lang,tb.key)}</span>
        </button>
      ))}
    </nav>
  );
...
// ─── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({cashBalance,totalBankBal,tax,data,profile,trialDaysLeft,onUpgrade,bankBalance}){
...
function TransactionBook({data,update,cashBalance,totalBankBal,bankBalance,profile}){
  ...
  const [form,setForm]=useState({medium:"cash",type:"income",amount:"",category:TXN_CATS_INCOME[0],description:"",date:today(),ref:"",accountId:"",isBusiness:false});
  ...
  const add=()=>{
    ...
    setForm({medium:"cash",type:"income",amount:"",category:TXN_CATS_INCOME[0],description:"",date:today(),ref:"",accountId:data.bankAccounts[0]?.id||"",isBusiness:false});
    setShowCustomCat(false);
    setShowForm(false);
  };
  const del=id=>{
  ...
  return(
    <div style={S.page}>
      <div style={S.pageHdr}><div style={S.pageTitle}>📒 Transaction Ledger</div><button style={S.addBtn} onClick={()=>{setShowForm(!showForm);setShowCustomCat(false);}}>{showForm?"✕ Cancel":"+ Add Transaction"}</button></div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}}>
                ...
                <span style={{fontSize:10,background:t.medium==="cash"?"#0f2d1f":"#0a1a3a",color:t.medium==="cash"?"#6ee7b7":"#93c5fd",padding:"1px 5px",borderRadius:3,fontSize:9,fontWeight:"700"}}>{t.medium==="cash"?"💵":"🏦"}</span>
...
// ─── INCOME PAGE ───────────────────────────────────────────────────────────────
function IncomePage({data,update,tax,profile,incomeCats}){
  const [showForm,setShowForm]=useState(false);
  ...
  return(
    <div style={S.page}>
      <div style={S.pageHdr}><div style={S.pageTitle}>📥 Income — {TAX_YEAR}</div><button style={S.addBtn} onClick={()=>setShowForm(!showForm)}>{showForm?"✕":"+ Add"}</button></div>
...
// ─── TAX PAGE ──────────────────────────────────────────────────────────────────
function TaxPage({data,update,tax,profile}){
  ...
  return(
    <div style={S.page}>
      <div style={S.pageTitle}>📋 Tax — {TAX_YEAR}</div>
      <div style={{fontSize:10,color:"#64748b",marginBottom:14}}>Inland Revenue (Amendment) Act No. 2 of 2025 · {isFreelancer?"Freelancer / Self-Employed Rules":"Salaried Employee Rules"}</div>
...
// ─── REPORTS PAGE ──────────────────────────────────────────────────────────────
function ReportsPage({data,tax,cashBalance,totalBankBal,profile,bankBalance}){
...
<div style="text-align:center;margin-top:16px"><button onclick="window.print()" style="background:#1a1a2e;color:#f5d060;border:none;padding:10px 24px;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer">🖨️ Print / Save as PDF</button></div>
  ...
  return(
    <div style={S.page}>
      <div style={S.pageTitle}>📊 Reports — {TAX_YEAR}</div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {["annual","monthly","cashflow"].map(v=>(
          <button key={v} style={{...S.tBtn,...(view===v?S.tBtnA:{})}} onClick={()=>setView(v)}>
            {v==="annual"?"📅 Annual":v==="monthly"?"📆 Monthly":"💹 Flow"}
          </button>
        ))}
      </div>
      {view==="annual"&&(
  ...
  return(
    <div style={S.page}>
      ...
      <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:6,marginBottom:14}}>
        {[["guide","📋","Guide"],["ird","🏛️","IRD Portal"],["steps","🪜","Steps"],["faq","❓","FAQ"],["contact","📞","Contact"]].map(([id,ico,lbl])=>(
